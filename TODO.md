...
- [ ] **Phase 1.5：場景描述與複合場景（2026-08-27 使用者需求新增）** —
      **T-20 ✅**（Opus 驗證 2026-08-28）/ **T-21 🟠 退回**（Opus 2026-08-28：
      巨蛋場景 2k/4k −94% 靜默錯誤，根因是 T-14 引擎固定早期窗）/
      **T-22 🔵 待驗證**（T-14 引擎尺度自適應，Sonnet 完成 2026-08-28：
      早期窗改依幾何動態計算、40–200m 尺度掃描 2k/4k 誤差全部收斂到 ≤25%，
      4×3×2.5／30×20×12 兩條交付 IR MD5 零回歸，新增能量匹配窗縱深防禦警示）
      **← 現在該做的：開 Opus 視窗驗證 T-22 → 接著跑 T-21 修正輪
      （export_coupled 警示串接＋重生 v4 試聽檔）→ 使用者重聽 v4 → Opus 複驗 T-21**
