...
- [ ] **Phase 1：MVP（照片 → IR 匯出）** — T-10 ✅ / T-12 ✅（Opus 驗證通過 2026-08-25）/
      **T-11 🔴 決策補丁步驟 7–8 完成、A'/B' 判準通過，但 Steinman 防濫殺對照組卡關
      ← 待使用者拿去問 Fable** / **T-13 🔵 Sonnet 完成、待 Opus 驗證**
...
- [ ] 🔴 **T-11 決策補丁待 Fable 確認（2026-08-26）** — 量程規則已實作、REPORT §8 已補、
      A'（浴室 +24%）/B'（走廊/車內/體育館全部 low）皆通過；但 Steinman 環景防濫殺對照組
      翻成 low（決策文字的粗估與實際六視角原始牆距不一致，3/4 面 >10m）。
      待 Fable 決定接受 low 或調整環景量程判定基準，才能改回 🔵 待驗證。
