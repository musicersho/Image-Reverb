...
- [ ] **Phase 1：MVP（照片 → IR 匯出）** — T-10 經 Opus 驗證 🟠 **退回**：
      前處理順序反了（先裁邊再判環景 → equirect 均勻極點被當黑邊裁掉，靜默破壞投影）。
      修好後才能開 T-11／T-12
...
- [ ] **T-10 🟠 退回修正（下一張卡，Sonnet）**：`is_equirect()` 改對原圖判斷、
      判定為環景就跳過黑邊裁切；加一個「極點均勻的合成 equirect」迴歸測試
- [ ] T-11 幾何估計模組 ｜ T-12 材質模組（T-10 修好後可並行）
