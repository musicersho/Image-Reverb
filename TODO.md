...
- [ ] **Phase 1：MVP（照片 → IR 匯出）** — T-10 ✅ / T-11 ✅ / T-12 ✅ / T-13 ✅ /
      **T-14 🔵 待驗證**（Fable 執行完成 2026-08-27，閉環迴歸 11 項全過，
      使用者試聽 ✅「目前聽起來 OK」）
- [ ] **Phase 1.5：場景描述與複合場景（2026-08-27 使用者需求新增）** —
      **T-20 🔵 待驗證**（文字場景→IR，13 preset）/ **T-21 🔵 待驗證**（複合場景
      路徑串接引擎＋巨蛋走廊、隔壁人聲兩個示範場景）
      **← 現在該做的：使用者試聽 4 個新檔 → 開 Opus 視窗依序驗證 T-14 → T-20 → T-21**
...
- [ ] 🎧 **第三輪試聽 T-21 的 2 個檔案（v3）**：
      `afplay output/listen_coupled_stadium_corridor.wav`（v3：滿場巨蛋，尾巴 12.9→5.1s）、
      `afplay output/listen_coupled_neighbor_voices.wav`（v3：低頻共振壓 ~7dB、中高頻更悶）
