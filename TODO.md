...
- [ ] **Phase 1.5：場景描述與複合場景（2026-08-27 使用者需求新增）** —
      **T-20 ✅**（Opus 驗證 2026-08-28）/ **T-21 🔵 待驗證**（修正輪完成＋使用者第四輪
      重聽 ✅ 2026-08-28：閉環比對警示接上、「尺寸無關」斷言更正、v4 重生——
      巨蛋 2k/4k 由 −94% 收斂到 −13%/−17%，neighbor_voices MD5 與 v3 相同）/
      **T-22 ✅**（Opus 驗證 2026-08-28；T-14 引擎尺度自適應：
      早期窗改依幾何動態計算、40–200m 尺度掃描 2k/4k 誤差全部收斂到 ≤25%，
      4×3×2.5／30×20×12 兩條交付 IR MD5 零回歸，新增能量匹配窗縱深防禦警示）
      **← 現在該做的：開**新的** Opus 視窗複驗 T-21
      （本輪修正是驗證 T-22 的那個視窗接手做的，別讓同一個上下文自己審自己）
      → 通過後 Phase 1.5 全數完成，回頭接 Phase 1 的 T-15/T-16/T-17**
...
- [ ] 小修（T-11 Opus 驗證建議，不阻擋）：`--override-dims` 與 `--geometry` 寫同一個
      ...
      → §7 調換順序或手動結果另存檔名；`apply_scope_confidence()` 改成「不認得的
      dims_source 就降 low」而非目前的預設放行（將來新增來源會靜默失效）；
      docstring 補一句 manual 路徑豁免量程規則
- [ ] 小修（T-10 殘留，不阻擋）：非環景裁切後若長寬比落回 2:1 容差內就印警告
      （防「帶外框的環景圖被靜默當一般照片」）；`view_el+45.png` 檔名去掉 `+`；
      可變物件當預設參數；傳目錄時的錯誤訊息語意
- [ ] T-14 → T-15 → T-16 → T-17（依序）
