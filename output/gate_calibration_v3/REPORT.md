...
|---|---|
| 跑法 | `--fresh`（零快取，全新目錄） |
| out_dir | output/gate_calibration_v3 |
| 照片張數（`t36.GATE_ITEMS`） | 13 |
## Provenance（只記錄、不比對）

| provenance（只記錄，不比對） | 值 |
|---|---|
| 主 repo HEAD（產生本報告時） | `c2a778e886fc7812097e47665677f30caba3dbb5` |
| git status --porcelain -- src scripts data | 見下方 |
| 產生時間（UTC） | 2026-09-15T05:38:22.768634+00:00 |
| 環境 | macOS-15.7.7-arm64-arm-64bit；python 3.9.6；torch 2.8.0 |
| code_fingerprint.repo_head（跑 CLI 當下） | `c2a778e886fc7812097e47665677f30caba3dbb5` |

`git status --porcelain -- src scripts data`：
```
 M src/image_reverb/geometry.py
?? scripts/test_geometry_scope.py
