...
|---|---|
| 跑法 | `--fresh`（零快取，全新目錄） |
| out_dir | output/gate_calibration_v2 |
| 照片張數（`t36.GATE_ITEMS`） | 13 |
## Provenance（只記錄、不比對）

| provenance（只記錄，不比對） | 值 |
|---|---|
| 主 repo HEAD（產生本報告時） | `7e03a97d2f96ae8c7d0d9023aa5b9ace1008d493` |
| git status --porcelain -- src scripts data | 見下方 |
| 產生時間（UTC） | 2026-09-14T06:52:54.414466+00:00 |
| 環境 | macOS-15.7.7-arm64-arm-64bit；python 3.9.6；torch 2.8.0 |
| code_fingerprint.repo_head（跑 CLI 當下） | `7e03a97d2f96ae8c7d0d9023aa5b9ace1008d493` |

`git status --porcelain -- src scripts data`：
```
 M scripts/test_confidence_axes.py
 M src/image_reverb/materials.py
 M src/image_reverb/pipeline.py
 M src/image_reverb/surfaces.py
...
② **被放行案例清單**：`default` 模式 0 面來自 pass 案例、`role_aware` 模式 0 面來自 pass 案例，逐面 vs ground truth 見 tables.md 表 2。

③ **T-17 已知錯誤（鐵則 12）輸出佔比**：5 張已知錯誤案例中，`default` 模式 0/5 張為 pass、`role_aware` 模式 0/5 張為 pass（見 tables.md 表 3）；pass 案例裡的 in-set 誤判面數／pass 案例總評分面數：`default` 0/0、`role_aware` 0/0。
...
⑦ 兩種唯讀模擬（只算不採用，`compute_materials_confidence()`／gate 判定段／門檻 0.4 零改動）：
  (a) 門檻依候選數 n 調整（等效全域 16 候選 softmax 的機率門檻）對 gate 的影響——見 tables.md 表 7；模擬後 pass 張數／實際 pass 張數：`default` 0/0、`role_aware` 0/0。
  (b) `compute_materials_confidence()` 規則 4 加「候選集收窄的 clip 面不得直接 medium」對 gate 的影響——見 tables.md 表 8；模擬後 pass 張數／實際 pass 張數：`default` 0/0、`role_aware` 0/0。
