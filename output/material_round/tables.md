...
計算方法：20ms 非重疊窗逐窗 RMS 包絡線；門檻相對整段訊號的絕對值峰值（取樣，非包絡線峰值）；對每個門檻取「最後一個超過門檻之窗」的**起始時間**。輸入是卷積後的試聽檔（`assets/dry/clap_synth.wav` mix=0.6），不是 `ir_mono.wav`。

| 門檻 | 有陳設 (`listen_bedroom_with_furnishings.wav`) | 無陳設 (`listen_bedroom_without_furnishings.wav`) | 差距 |
|---|---|---|---|
