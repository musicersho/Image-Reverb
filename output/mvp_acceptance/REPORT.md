...
**§7-1 的盲聽以完全獨立的方式撞出同一個結論**：使用者把體育館聽成車內、把臥室聽成教堂
——**大小被聽反了**。查證後，使用者的耳朵每一題都正確描述了聽到的聲音，**錯的是管線**
（體育館被估成 30.8 m³、車內被估成 332 m³）。詳見 §1.2。
...
- **`sample_4` 臥室 → 3.56 秒殘響（🔴 最危險的一筆）**：這是**唯一沒有被攔下來的**——
  ...
  病因是材質：六個面被判成 `generic_wall`×4 ＋ `gypsum_board`×2，
  **1kHz 吸音係數只有 0.03–0.04**。真實臥室有床、地毯、窗簾（α 0.37–0.72），
  實際殘響應該在 0.4 秒量級。**再次是材質，與 §2.4 的壁球場同一個病因。**

### 1.3 ⚠️ 機制觀察：CLIP 不確定時的 fallback 是全表最不吸音的材質

`sample_4` 的地板 CLIP top-1 機率只有 0.24（低於 0.4 門檻）→ fallback 到 `gypsum_board`。
...
| **`generic_wall`（fallback）** | **0.03** | | `curtain_fabric` | 0.72 |
| **`gypsum_board`（fallback）** | **0.04** | | `acoustic_panel` | 0.85 |
| `brick` | 0.04 | | `audience_seating` | 0.96 |

**12 種材質裡，兩個 fallback 值排在最不吸音的第 2、第 4 名。**
「模型不知道」時的預設，在聲學上等於「假設這個面完全不吸音」，方向上必然把殘響推長。

> **但這條相關性在本資料集上證據不足，必須誠實說明**：拿 §7-2 八個場地的
> 「fallback／域外面數」對「聯合帶誤差」算相關係數，**含沙灘 r = −0.57、
> 去掉沙灘 r = +0.90**——n=8 拿掉一個點就整個翻向。
> **這種相關係數不能當證據**，本報告不據此宣稱 fallback 是系統性高估的主因。
> 站得住的是①上面那張 α 表（不需統計，直接讀數字）與②`sample_4`、壁球場兩個
> 已被隔離的個案。**建議把「fallback 材質的預設值選擇」列為獨立議題查證，而不是
> 靠這份資料下結論。**
...
0. **§7-1 與 §7-2 是兩條互相獨立的證據鏈，卻指向同一個結論。**
   §7-2 靠 8 個場地的 RT60 量測、§7-1 靠使用者的耳朵，兩邊都指到材質：
   壁球場（量測，−50%→+13%）與臥室（聽感，被聽成教堂；六面全是 α 0.03–0.04 的
   fallback 材質）。**兩條獨立路徑撞出同一個病因，這比任何單一證據都強。**
...
6. **系統性偏長是跨量測與聽感的一致訊號**（§4.3）：§7-2 多數場地高估、§7-4 聽感也說過長。
   比單一場地的數字更值得優先處理。
7. **fallback 材質的預設值選擇值得單獨查證**（§1.3）：`gypsum_board`／`generic_wall`
   是 12 種材質裡第 2、第 4 不吸音的。機制清楚，但**本資料集的相關性證據不足**
