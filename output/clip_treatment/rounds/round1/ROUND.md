# Round `round1` — T-38 治療輪紀錄（歷史補寫，T-38A 步驟 4）

> 本檔由 T-38A 補寫，不是當輪執行當下產生的即時紀錄——round1 是「上一個視窗」
...
- status: complete
- 父輪次: round0_baseline（推定——逐輪覆寫的線性紀錄推定，非精確 lineage 證據）
- 執行指令: `python scripts/t38_treatment_eval.py round1`（重建的慣例格式，非逐字保留的原始指令列）
...
（`72ec1b8d1e572f0c0b1c92f1b9768c2698dcab8b9a4d5916930cf7a0bb6c04d3`）與
...
相對 round0_baseline（31/76、4/13、9）：overall 下降、floor 持平、in-set 誤判上升
——不符合產品採用門檻（原 T-38 卡與 T-38B 卡繼承的門檻）。

## 與 T-33 凍結快取的差異範圍評估

非預期範圍：10 張照片有差異（CathedralRoom, DivorceBeach, RacquetballCourt4,
SteinmanHall, TunnelToHell, bathroom_tiled, bedroom_ai_generated, car_interior_suv,
site_photo_gym, site_photo_restaurant），其中 9 張非 TunnelToHell，多半是本輪提示詞
造成的漂移（`diff_scope_summary()` 對本輪差異集合的實際輸出，見同目錄 `tables.md`
最後一節的完整差異清單）。
...
| 項目 | 值 |
|---|---|
| code_sha256 | {"preprocess.py": "f3d1d2f820087f603ff5cde90c6a4905ec87513fd3d846440c2f79ee7f6b8352", "surfaces.py": "72ec1b8d1e572f0c0b1c92f1b9768c2698dcab8b9a4d5916930cf7a0bb6c04d3", "config.py": "c1e09da90283cb20b3a1c672f0b3150c6f6c280d6d83adeeb218b6413fc176c4"} |
