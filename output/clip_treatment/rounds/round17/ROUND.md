# Round `round17` — T-44 role-aware 分區輪紀錄

- status: complete
- 父輪次: round16
- 執行指令: `python scripts/t44_role_eval.py round17 --hypothesis PLAN_T44.md §8：撤銷 round16 對 floor 加回 generic_wall 的改動（證偽：它自己接管成新錯誤冠軍而非稀釋，且讓 stairwell_tiled.floor 倒退），floor 還原成 round15 的 6 種候選；wall 保留 round16 的還原（全域 12 種）；ceiling 不動。本卡最後一輪（round15 首輪＋round16／round17 兩輪調整＝預算用滿） --parent round16 --role-sensitivity`
- 本輪假設與修改理由: PLAN_T44.md §8：撤銷 round16 對 floor 加回 generic_wall 的改動（證偽：它自己接管成新錯誤冠軍而非稀釋，且讓 stairwell_tiled.floor 倒退），floor 還原成 round15 的 6 種候選；wall 保留 round16 的還原（全域 12 種）；ceiling 不動。本卡最後一輪（round15 首輪＋round16／round17 兩輪調整＝預算用滿）
...
overall 32/76 （上升 相對 30）；floor 5/13 （上升 相對 4）；in-set 誤判 8 （下降 相對 9）。

## 按判定來源分組

| 來源 | 面數 | 排除數 | 正確率 |
|---|---|---|---|
| clip | 23 | 1 | 14/22（63.6%） |
...
| 項目 | 值 |
|---|---|
| code_sha256 | {"preprocess.py": "f3d1d2f820087f603ff5cde90c6a4905ec87513fd3d846440c2f79ee7f6b8352", "surfaces.py": "9125d64a5bae93441e1ce9c1ba6defb9aac5cb8151c3b5db46e4297dfe440bf4", "config.py": "c1e09da90283cb20b3a1c672f0b3150c6f6c280d6d83adeeb218b6413fc176c4"} |
...
| clip_threshold | 0.4 |
| eval_mode | t44_role_aware:round17 |
