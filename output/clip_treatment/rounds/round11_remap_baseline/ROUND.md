# Round `round11_remap_baseline` — T-38 治療輪紀錄

- status: complete
- 父輪次: round0_baseline
- 執行指令: `python scripts/t38_treatment_eval.py round11_remap_baseline --hypothesis 16 個 proxy 面經使用者逐面重對映後、候選集未動（仍是原 12 條 CLIP_MATERIAL_PROMPTS）的全量重跑，作為 T-39 兩段式基線的第一段。預期整體正確率下降（3 面新標的真實材質尚不在候選集內，必錯），其餘 13 面沿用原候選不變。 --parent round0_baseline`
- 本輪假設與修改理由: 16 個 proxy 面經使用者逐面重對映後、候選集未動（仍是原 12 條 CLIP_MATERIAL_PROMPTS）的全量重跑，作為 T-39 兩段式基線的第一段。預期整體正確率下降（3 面新標的真實材質尚不在候選集內，必錯），其餘 13 面沿用原候選不變。
...
| data_sha256 | {"materials.json": "8afb86e4a44cbb85fc274b1f9369aa598c816a8f1bb60a0f612e5007cd48127f", "material_ground_truth.json": "965e51ac19e2d25a61b89bb8b94c01e4f34e3e41d6300002627d212abfc430c7"} |
| segmentation_model_id | nvidia/segformer-b4-finetuned-ade-512-512 |
| clip_model_id | openai/clip-vit-base-patch32 |
| clip_threshold | 0.4 |
| eval_mode | treatment:round11_remap_baseline |
