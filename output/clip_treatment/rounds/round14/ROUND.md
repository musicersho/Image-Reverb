# Round `round14` — T-38 治療輪紀錄

- status: complete
- 父輪次: round13
- 執行指令: `python scripts/t38_treatment_eval.py round14 --hypothesis PLAN_T39.md §7 事前規則：round13 修正 metal_roof_deck 後，唯一剩下與新候選相關的搶答是 vinyl_panel 偷走 RacquetballCourt4.south（原正確 glass，信心 0.548）。vinyl_panel 自身目標面（bathroom_tiled.ceiling）在分割階段就未被偵測到，字串調整對它自己的目標無效，本輪只驗證能否至少壓下這個副作用（本卡最後一輪調整，跑完即用完 2 輪調整預算）。假設：原字串『glossy...non-porous』的 glossy 與玻璃光澤語意重疊；改法：明示『opaque』『not glass and not transparent』，只改 vinyl_panel 一個候選（單一變因，累積 round13 的 metal_roof_deck 改動）。 --parent round13`
- 本輪假設與修改理由: PLAN_T39.md §7 事前規則：round13 修正 metal_roof_deck 後，唯一剩下與新候選相關的搶答是 vinyl_panel 偷走 RacquetballCourt4.south（原正確 glass，信心 0.548）。vinyl_panel 自身目標面（bathroom_tiled.ceiling）在分割階段就未被偵測到，字串調整對它自己的目標無效，本輪只驗證能否至少壓下這個副作用（本卡最後一輪調整，跑完即用完 2 輪調整預算）。假設：原字串『glossy...non-porous』的 glossy 與玻璃光澤語意重疊；改法：明示『opaque』『not glass and not transparent』，只改 vinyl_panel 一個候選（單一變因，累積 round13 的 metal_roof_deck 改動）。
...
```json
{
  "CLIP_MATERIAL_PROMPTS": {
    ...
    "vinyl_panel": "a smooth opaque plastic or vinyl panel surface with a slight sheen, not glass and not transparent",
...
- CLIP_MATERIAL_PROMPTS.vinyl_panel：新增（"a smooth opaque plastic or vinyl panel surface with a slight sheen, not glass and not transparent"）
...
| 項目 | 值 |
|---|---|
| code_sha256 | {"preprocess.py": "f3d1d2f820087f603ff5cde90c6a4905ec87513fd3d846440c2f79ee7f6b8352", "surfaces.py": "7b6c4c67e72e45119c91205117f9a7d4d59469cdfa5429d82a94e9fcf8e408c7", "config.py": "c1e09da90283cb20b3a1c672f0b3150c6f6c280d6d83adeeb218b6413fc176c4"} |
