## 表 1：13 張照片基準率變化（預設 vs --role-aware，vs 基線 B0＝commit `23f2aba`）

| 照片 | geometry（預設） | geometry（--role-aware） | materials（預設） | overall（預設） | gate（預設） | materials（--role-aware） | overall（--role-aware） | gate（--role-aware） | 與 B0 相符 | 與 round11 相符 | 與 round17 相符 |
|---|---|---|---|---|---|---|---|---|---|---|---|
...
- TunnelToHell：預設模式 geometry_confidence=low 與 EXPECTED_GATE（T-28-A／T-36 凍結表）medium 不同（B0 於 23f2aba 亦同，非本卡回歸）
