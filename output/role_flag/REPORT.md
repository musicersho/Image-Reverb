...
| 欄位 | 值 |
|---|---|
| 主 repo HEAD（產生本報告時） | `c8f6be9b60fbb7030a27687436c4dd5fbe97c495` |
| B0 commit（全長） | `23f2aba` → `23f2abada92aa9d46b1da0ac1ba2a7f1dc178872` |
| criteria_version | v2 |
| criteria_commit（本檔 commit） | `2be24530766e14951bd4562f6c5f87d33555bd63` |
| 跑法 | `--fresh`（含 B0 重建，零快取） |

本報告由 `scripts/t46_role_flag_baseline.py` 依 [`CRITERIA_T46_v2.md`](CRITERIA_T46_v2.md) 對 13 張照片各跑一次真實 CLI（`python -m src.image_reverb <photo> --force-low-confidence --no-viz`，預設模式與加 `--role-aware` 各一次），並在 `git worktree` 重建基線 B0（commit `23f2aba`：role_aware 尚未預設啟用的最後狀態）之真實 CLI 結果，程式化驗證：
...
13 張全數通過（13/13 三項比對皆相符：與 B0、與 round11、與 round17）。完整表格見 [`tables.md`](tables.md)、B0 的持久證據見 [`baseline_23f2aba/BASELINE.md`](baseline_23f2aba/BASELINE.md)。
