...
- 改動後主 repo `git rev-parse HEAD`（全長）：`0d800c303d70c5b563311b3e622efca5aaef13a7`
- 改動後主 repo `git status --porcelain -- src scripts data`：
```
 M scripts/t17_blind_test.py
 M src/image_reverb/pipeline.py
?? scripts/t43_provenance_baseline.py
?? scripts/test_t17_provenance.py
?? src/image_reverb/provenance.py
```
⚠️ 改動後為未 commit 工作區（執行者本次跑的當下工作區有未 commit 變更，不因此視為斷言失敗；Opus 複驗那次應為空）。
- 產生時間（UTC）：`2026-09-11T07:44:32Z`
