...
def _publish_staging(stem: str, staging_root: Path, archive_dir: Path | None = None) -> None:
    ...
    try:
        ...
        print(
            f"⚠️ 發布中斷：{staging_root} 可能處於半發布狀態，"
            "請手動檢查並搬移到正式位置"
            + (f"（本次執行前的舊輸出仍隔離於 {archive_dir}）" if archive_dir else "")
            + "。",
            file=sys.stderr,
        )
        raise
...
def run_photo(
    ...
    # T-49（裁決 T-42-A 執行卡 1/2，附帶發現②）：run_photo() 的第四個出口——
    # 非預期例外（例：_run_wet_preview() 的 subprocess.CalledProcessError）。
    # 既有三個出口（exit 2 ×2、exit 3）各自的 stderr 文字／rmtree 保留在原本
    # 的位置不動；這層 try/finally 只是兜底：任何沒被既有 except 攔到、也還沒
    # 發布成功的路徑，finally 都會清掉 staging，並且只在前面三個出口都還沒印過
    # archive_note 時才補印一次——不然使用者只看到 traceback，不知道舊輸出被
    # 搬去哪、怎麼拿回來。例外本身照樣原封往上拋，不吞、不改 exit code
    # （WORKFLOW §5 紅旗 4）。
    published = False
    archive_note_printed = False
    try:
        try:
        ...
        # 新舊混合的半套。
        published = True
        _publish_staging(stem, staging_root, archive_dir)
        ...
        if not published:
            if archive_note and not archive_note_printed:
                print(archive_note, file=sys.stderr)
            shutil.rmtree(staging_root, ignore_errors=True)
