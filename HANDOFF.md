...
> 最後更新：2026-08-27（Fable 執行 T-14/T-20/T-21 視窗）｜對應 commit：見 `git log --oneline -1`
> **新視窗請先讀 [CLAUDE.md](CLAUDE.md) 知道自己的角色，再讀本檔知道現在的狀況。**
>
> 驗證本檔是否過期：看 [DEV_LOG.md](DEV_LOG.md) 最上面一筆是不是 `2026-08-27 (26)`；
...
**Phase 0 已結案，現在是 Phase 1＋Phase 1.5。**
**T-10 ✅｜T-11 ✅｜T-12 ✅｜T-13 ✅｜T-14 🔵 待驗證（使用者試聽 ✅「目前聽起來 OK」）｜
T-20 🔵 待驗證（文字場景→IR）｜T-21 🔵 待驗證（複合場景引擎 v1）。**

🎯 **現在該做的：(1) 使用者第三輪試聽 T-21 的 2 個 coupled 檔（v3，清單在 TODO.md）；
(2) 開 Opus 視窗依序驗證 T-14 → T-20 → T-21（T-20/T-21 建立在 T-14 引擎上，先驗地基；
Prompt 在 §7，任務編號換掉即可）。**
試聽進度：T-14 ✅「目前聽起來 OK」｜T-20 ✅「沒有問題」｜T-21 兩輪退回迭代中
（v1 乾聲混入/場景平衡 → v2 巨蛋尾太長/低頻共振 → v3 滿場巨蛋＋eq_db 調音，待複聽）。
人耳回饋全文與修正記在 TASKS.md T-21 卡。
Phase 1.5 是使用者 2026-08-27 新增的需求（文字描述場景＋複合場景），
規劃決策見 DEV_LOG (25)、SPEC v0.4 §3.3、TASKS.md T-20/T-21 卡。
...
≤20%，實測 +1.1%）」。完整證據與逐成分分解在 TASKS.md T-14 卡，Opus 驗證時請
直接對裁決的證據鏈做對抗檢查（紅旗也列在卡上）。

🔮 **T-11 已於 2026-08-27 通過 Opus 驗證**（全文在 TASKS.md T-11 卡「Opus 驗證結果」）。
量程規則（`GEOMETRY_SCOPE_MAX_M = 10.0`）確認為資料＋config 驅動而非 hardcode 場地名——
驗證時把門檻改成 30.0，走廊就從 low 翻回 medium；改 5.0 維持 low。環景「看單面牆距而非
相加總長」也做了對抗測試（四面各 9.9m、相加 19.8m → medium；單面 10.1m、相加 15.4m → low）。
全部 10 個場地重跑，數字與 REPORT §8.2 逐位吻合；三個門檻查全 git 歷史都沒被動過。
**環景解「視野外」、不解「量程」。**

⚠️ **T-11 驗證踩到的陷阱（下個視窗要知道）**：REPORT §7 的重跑指令最後一條是
`--override-dims`，它和 `--geometry` **寫同一個 `output/geometry/<場地>/geometry.json`**，
所以照 §7 順序跑完，浴室的佐證檔會停在 `manual`/`high`，看不到 +24%/medium 的證據。
重跑 `--geometry` 即可重現。四個不阻擋的建議都記在 T-11 卡「Opus 建議」。
...
>    請對裁決的證據鏈做對抗檢查：機制閉環（平坦目標）若不達標，裁決不成立，直接退回。
> 2. 反造假紅旗都列在卡上：量測程式獨立性、量測窗不得只量尾巴、crossfade 能量跳變。
>    晚期 noise 是固定 seed（`config.IR_NOISE_SEED`），乾淨重跑可直接比 MD5。
> 3. `python scripts/test_ir_synth.py` 是 11 項閉環迴歸，任何 clone 可跑（不依賴模型下載）。
...
| T-14 | IR 合成引擎 v1（早期 image-source + 晚期 shaped-noise） | 🔵 **待驗證**（Fable 執行 2026-08-27；閉環迴歸 11 項全過；使用者試聽 ✅） |
| T-20 | 文字場景描述 → IR（13 preset＋解析器，F-16） | 🔵 **待驗證**（Fable 執行 2026-08-27；迴歸 13 項全過；等使用者試聽） |
| T-21 | 複合場景引擎 v1（路徑串接＋TL 表，F-17） | 🔵 **待驗證**（Fable 執行 2026-08-27；迴歸 12 項全過；等使用者試聽） |
| T-15~T-17 | Phase 1 其餘任務（已細化） | ⬜ 未開始（T-15 等 T-14/T-20/T-21 過驗證後整合三種輸入） |
...
【現在該做的 — 驗證 T-14】（T-14 已由 Fable 執行完畢，2026-08-27。模型選 Opus，另開視窗）
  貼：「你是驗證者，只審查不修改程式碼。請依 WORKFLOW.md 第 5 節的驗證標準，
      審查任務 T-14 的成果：讀 TASKS.md 的該任務卡、讀相關程式碼、實際執行驗證指令。
      最後輸出：「✅ 通過」或「❌ 退回」＋具體理由。」
```

### ⚠️ 給下一個視窗的提醒

- **T-14 是直接生成 IR 的卡** → SPEC §7 第 4 條（人耳試聽驗收）一定會用到。
  數字合理 ≠ 聽起來對，本專案已經吃過一次虧（地雷 #9「鐵筒子」）。
- **地雷 #14**：Sabine 公式值與實測 IR 在低頻差 2 倍以上，T-14 要正面處理，
  不要把 T-13 的公式值當成使用者會聽到的結果。
- 新增 zero-shot 分類的地方必讀地雷 **#13**（softmax 無法表達「以上皆非」）。
- 驗證 T-11/T-13 時各發現一個「靜默失效」的潛在缺口（不阻擋，已記在各自任務卡）：
  T-13 的零/負尺寸、T-11 量程規則的預設放行。T-15 整合時一併補防呆。
