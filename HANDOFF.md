...
> 最後更新：2026-08-31（Fable 視窗：T-33 通過後複評裁決 T-33-A——陳設改預設
> 觀測模式、開 CLIP 準確度診斷輪、gate 規則不動但依據改寫；開 Phase 1.8 輪）
> **新視窗請先讀 [CLAUDE.md](CLAUDE.md) 知道自己的角色，再讀本檔知道現在的狀況。**
>
> 驗證本檔是否過期：看 [DEV_LOG.md](DEV_LOG.md) 最上面一筆是不是 `2026-08-31 (66)`；
...
🎯 **現在該做的：Sonnet 執行 T-33 文件修正（純文件小步）→ T-34**。
...
| T-33 | 材質輪基準率複測（量測卡） | ✅ **通過**（Opus 驗證 2026-08-31；量測結論：陳設機制淨效果為負）｜🚧 兩則文件修正待補 **← 現在做這個** |
| — | 🔮 裁決 T-33-A（T-33 複評＋Phase 1.8 規劃） | ✅ **已裁決**（Fable 2026-08-31：陳設改觀測模式、開 CLIP 診斷輪、gate 依據改寫；全文 TASKS.md T-33 卡尾） |
| T-34 | gate 訊息補洞（規則 2 死路＋測試覆蓋） | ⬜ 未開始（裁決 T-33-A 確認照原卡執行，T-33 文件修正後接著做） |
| T-35 | 陳設改預設觀測模式（裁決 T-33-A 裁決 A） | ⬜ 未開始（T-34 後） |
| T-36 | CLIP 材質判定準確度診斷（量測卡，需使用者標註） | ⬜ 未開始（T-35 後；交回時 gate 規則就地定案） |
...
【現在該做的 — 開 Sonnet 視窗做 T-33 文件修正】（模型選 Sonnet）
  純文件小步：照 TASKS.md T-33 卡「Opus 驗證紀錄」修正事項一／二逐字執行
  （①REPORT §2.2/§4.3 的 8/13 → 9/13；②§6.1 門檻衰減表補程式來源進腳本與
  data.json，或明確標注人工查核方法），commit `docs: T-33 文件修正`。

【接著 — Sonnet 依序執行 T-34 → T-35】（模型選 Sonnet）
  貼 WORKFLOW §2.1 標準 Prompt。T-34（gate 訊息規則 2 死路出口，照原卡逐字
  執行）；T-35（陳設改預設觀測模式，裁決 T-33-A 裁決 A 執行卡）。
  每張做完各開 Opus 視窗驗證（WORKFLOW §2.2）。

【然後 — Sonnet 執行 T-36（CLIP 準確度診斷，量測卡）】（模型選 Sonnet）
...
- **T-33 的意外發現（已裁決處理）**：陳設等效吸音對 §7-2 式達標率是**淨負面**
  （自動組 22%→10%、手動組 20%→12%，根因見地雷 #26）。裁決 T-33-A 已定案改
  **預設觀測模式**（執行卡 T-35）；在 T-35 落地前，程式的現狀仍是預設套用——
  重跑照片 IR 比對數字時要知道自己在哪個狀態（`--no-furnishings` 組＝T-17 基準）。
