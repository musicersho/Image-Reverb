...
> 最後更新：2026-08-30（Fable 規劃視窗：第 0 節 A~F 六項全數裁決，
> T-15/T-16/T-17 卡改版、新增 T-18、§7-2 低頻判準事前裁決）
> **新視窗請先讀 [CLAUDE.md](CLAUDE.md) 知道自己的角色，再讀本檔知道現在的狀況。**
>
> 驗證本檔是否過期：看 [DEV_LOG.md](DEV_LOG.md) 最上面一筆是不是 `2026-08-30 (36)`；
...
重新規劃；現在做 Phase 1 的最後四張卡。**
**T-10 ✅｜T-11 ✅｜T-12 ✅｜T-13 ✅｜T-14 ✅｜T-20 ✅｜T-21 ✅｜T-22 ✅
（全部經 Opus 驗證）｜T-15 / T-16 / T-18 / T-17 ⬜ 未開始（依此順序；
T-18 可提前插入，T-17 前必過）。**

🎯 **現在該做的：開 Sonnet 視窗執行 T-15**（WORKFLOW §2.1 標準 Prompt）。
Fable 已於 2026-08-30 完成重新規劃：T-15/T-16/T-17 卡片依 Phase 1.5 後的實況改版、
新增 T-18（驗收前置卡）、T-17 §7-2 低頻判準完成事前裁決。六項裁決摘要見第 0 節，
細節在 TASKS.md 各卡的 🔮 記錄。**T-18 不依賴 T-15/T-16、可隨時插入，
但必須在 T-17 開工前通過 Opus 驗證。**
...
| T-15 | CLI 整合（照片／文字／複合場景 → IR + 報告） | ⬜ **未開始（下一步；卡片已於 2026-08-30 改版完成）** |
| T-16 | 分析視覺化（三種輸入各有拼版） | ⬜ 未開始 |
| T-18 | 驗收前置（聯合帶量測工具＋退出碼技術債，2026-08-30 新增） | ⬜ 未開始（不依賴 T-15/T-16，T-17 前必過） |
| T-17 | MVP 驗收（SPEC §7 四項） | ⬜ 未開始（§7-2 低頻判準已於 2026-08-30 事前裁決，見 TASKS.md T-17 卡裁決 B） |
...
【現在該做的 — 開 Sonnet 視窗執行 T-15】（模型選 Sonnet）
  貼：「執行 TASKS.md 的任務 T-15。先讀 CLAUDE.md 和該任務卡的全部內容再動工，
      完成後執行任務卡裡的自我檢查，最後照 WORKFLOW.md 第 4 節做收工程序。」

【T-15 做完後】（模型選 Opus）貼 WORKFLOW §2.2 驗證 Prompt。
之後依序：T-16 → T-18 → T-17（T-18 可提前插入，但 T-17 前必須通過驗證）。
```

### ⚠️ 給下一個視窗的提醒

- **卡片已改版（2026-08-30）**：T-15 現在是三種輸入的統一入口卡，含技術債收斂與
  MD5 零回歸判準——逐字照卡片做即可，不要沿用對舊版卡的印象。
- **T-17 §7-2 的判準已事前裁決**（第 0 節 B）——驗收現場任何再改判準的提案
  一律視為 WORKFLOW §5 紅旗 3。
