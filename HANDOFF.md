...
>
> 驗證本檔是否過期：看 [DEV_LOG.md](DEV_LOG.md) 最上面一筆是不是 `2026-08-18 (14)`；
...
**T-10 ✅ 通過｜T-12 🔵 待 Opus 驗證（自檢全過，含使用者試聽）｜T-11 🔴 卡關 ← 現在的瓶頸。**

🎧 **好消息：「鐵筒子」缺陷正式結案。** 使用者 2026-08-18 實聽確認「沒問題」。
這是本專案第二次由人耳確認成果（第一次是 T-02），意義在於**約束 A 的實證閉環完成**：
2026-08-16 那個「像用手拍鐵筒子」的錯誤通過了 WORKFLOW §5 全部三層數值檢查、只有耳朵抓得到；
這次的量化改善（125Hz T30 **3.952s→0.748s**、低/高頻比 **48.8→1.27 倍**）**與聽感一致**。

🔴 **現在唯一的瓶頸是 T-11 路線決策，需要 Fable。下面第 0 節就是給你直接貼的 Prompt。**

---

## 0. 【現在該做的事】開 Fable 視窗決策 T-11 路線

> **模型選 Fable**，把下面整段貼進去：

```
你是規劃者。請讀 CLAUDE.md、SPEC.md、ROADMAP.md、TASKS.md 的 T-11 卡、
以及 output/geometry/REPORT.md（完整誤差表與 5 個候選方向），
然後決策 T-11 的路線，更新 TASKS.md 與 SPEC/ROADMAP（如需要），並在 DEV_LOG.md 記錄決策理由。

目前的情況是：T-08 決策一選的 metric depth 路線，在 T-11 內建的評測關卡上未通過。
根因不是實作問題，是模型量程——Depth-Anything-V2-Metric-Indoor-Small 在 9 張測試照片上
最大預測距離全部落在 3.6–19.7m。浴室（3m）誤差 +24% 通過判準，
但走廊（30m）−57%、體育館（150m）模型全圖最遠只說 3.61m（−98%）。
判準 B（車內與超大空間必須標 confidence: low）已通過。
T-13 之後的卡全部卡在這個決策上，請先定案尺寸來源。
```

### 給 Fable 的一分鐘摘要（詳細版在 REPORT.md）

**已經是事實、不需要再驗證的：**

| # | 事實 | 證據 |
|---|---|---|
| 1 | metric-indoor-Small 有效量程約 **0–20m**，超大空間結構上無法表達 | 9 張照片最大預測距離 3.6–19.7m；體育館實際 150m、模型說 3.61m |
| 2 | 量程內的小房間**可用** | 浴室 +24%，在 ±30% 判準內 |
| 3 | **不是**用錯模型或單位 | metric 版與相對版輸出確實不同；浴室 p50=2.94m 對照實際 2.5–3.5m 合理 |
| 4 | 深度統計**無法**偵測自己的失敗 | 體育館錯 98% 但 clamp 比例 0、百分位平順、離上限很遠 |
| 5 | 能偵測失敗的訊號在**分割**不在深度 | 地板可見度 0.0%（vs 浴室 6.8%）、人群佔比、CLIP 域外判定；已實作三條規則 |
| 6 | 手動覆寫（F-09）**已經可用** | `--override-dims 4x3x2.5`，標記 `dims_source: manual`，完全不跑模型 |

**5 個候選方向**（完整版含成本/風險在 [`output/geometry/REPORT.md`](output/geometry/REPORT.md) §6）：

| | 方向 | 一句話 |
|---|---|---|
| a | 換大模型 | `Metric-Indoor-Large`（~1.3GB，需再次授權下載）；**量程問題可能仍在**，天花板是訓練資料決定的 |
| b | 縮小適用範圍 | SPEC 限定「一般室內小空間（≲10m）」，超出就要求手動輸入；誠實可行，但 SPEC §7 的 8 個驗收場地會有一半落在範圍外 |
| c | 手動尺寸升為主路線 | 深度只給初始猜測；已可用，但偏離「照片自動分析」的願景 |
| d | 改用環景為主 | 環景對牆相加的幾何明顯較好（Steinman Hall 估 17.5×21.5×6.9m 量級合理），但 **5 張環景只有 1 張測得到，證據不足** |
| e | 放寬判準 | ⚠️ **不建議**：±30% 是 SPEC F-02 的目標，且 RT60 ∝ 體積，尺寸誤差會被立方級放大 |

⚠️ **本輪沒有為了通過而放寬任何判準**：`GEOMETRY_ERROR_TOLERANCE` 仍 0.30、
`CLIP_CONFIDENCE_THRESHOLD` 仍 0.4。Fable 若要調整，請當成明確的決策記錄下來。

### 決策完之後的下游影響（Fable 要一起處理）

- **T-13（聲學參數）目前擋著不能開工** —— RT60 ∝ 體積，尺寸來源沒定案會白做。
- T-13 另有一個本輪實測發現要納入設計：**Sabine 與實測 IR 在 125Hz 差 2 倍以上**
  （見第 6 節地雷 #14），若只輸出 Sabine 數字會與使用者實際聽到的不符。
- **T-12 不受這個決策影響**，可以另開 Opus 視窗平行驗證。
...
| T-12 | 材質模組（逐表面材質） | 🔵 **待 Opus 驗證**：自檢全過（**含使用者試聽 ✅**） |
...
【現在該做的 — 決策 T-11 路線】（模型選 Fable）
  貼本檔第 0 節那段 Prompt（已針對這次的卡關情境寫好，可直接複製）

【可平行做 — 驗證 T-12】（模型選 Opus）
  貼：「你是驗證者，只審查不修改程式碼。請依 WORKFLOW.md 第 5 節的驗證標準，
      審查任務 T-12 的成果：讀 TASKS.md 的該任務卡、讀相關程式碼、實際執行驗證指令。
      最後輸出：「✅ 通過」或「❌ 退回」＋具體理由。」
  ⚠️ T-12 卡明列的 Opus 驗證重點：per-wall 材質真的逐面傳進 pyroomacoustics
     （紅旗：先把六面 α 平均再套用）、CLIP 是真的跑模型而非 hardcode、信心 gating 有警示輸出。

【Fable 決策完之後 — 繼續執行】（模型選 Sonnet）
