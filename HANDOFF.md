...
> 最後更新：2026-08-31（Sonnet 視窗：**T-37 ✅、T-40 ✅ 皆已通過（Opus
> 驗證），T-41 透視照 SegFormer 重複載入去重完成自檢，🔵 待驗證**——
> `pipeline.py` scene_cues 段改重用 `surfaces_from_preprocess()` 已算好的
> `detail["class_ratios"]["single"]`，刪掉重複的 `_load_segmenter()`／
> `segment_roles()`；13 張基線零漂移、9 張透視照耗時全部改善；下一步是
> Opus 驗證 T-41，通過後開 T-38）
> **新視窗請先讀 [CLAUDE.md](CLAUDE.md) 知道自己的角色，再讀本檔知道現在的狀況。**
>
> 驗證本檔是否過期：看 [DEV_LOG.md](DEV_LOG.md) 最上面一筆是不是 `2026-08-31 (82)`；
...
🎯 **現在該做的：T-37 ✅、T-40 ✅ 皆已通過（Opus）；T-41（透視照 SegFormer
去重）已完成自檢，🔵 待驗證 → 開 Opus 視窗驗證 T-41，通過後開 Sonnet 視窗
執行 T-38**。⚠️ 2026-08-31 Fable 已依外部掃描報告在
Phase 1.9 插入**產物可信度修正輪 T-40～T-43**（評測快取指紋／SegFormer
去重／gate 交易式輸出／T-17 產物溯源——五項缺陷全部對碼核實屬實），完整
順序更新為
**T-37 ✅ → T-40 ✅ → T-41（🔵 待驗證）→ T-38 → T-39 → T-42 → T-43 → 收尾複評**；
...
| T-41 | 透視照 SegFormer 重複載入去重（插卡 2/4） | 🔵 **待驗證**（Sonnet 自檢通過，2026-08-31；`pipeline.py`） |
...
【Phase 1.9 跑完後 — 回 Fable 收尾複評】（模型選 Fable）
  帶 T-37/T-38/T-39 的 REPORT 與基線變化表＋插卡輪 T-40～T-43 的 REPORT，
  一次議決：治療效果總結（對照理論上限 7/13）、要不要開 MINC/DMS 模型卡、
  要不要開陳設公式修正輪（裁決 T-36-A 裁決三的重啟評估點）、T-17 複驗時機
