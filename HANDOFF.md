...
> 最後更新：2026-08-28（Opus 驗證視窗：T-21 複驗通過，Phase 1.5 結案）｜
> 對應 commit：`5da02c5 T-21: 驗證通過`
> **新視窗請先讀 [CLAUDE.md](CLAUDE.md) 知道自己的角色，再讀本檔知道現在的狀況。**
>
> 驗證本檔是否過期：看 [DEV_LOG.md](DEV_LOG.md) 最上面一筆是不是 `2026-08-28 (35)`；
...
**Phase 0 已結案；Phase 1.5 已於 2026-08-28 全數結案；現在回頭做 Phase 1 的最後三張卡。**
**T-10 ✅｜T-11 ✅｜T-12 ✅｜T-13 ✅｜T-14 ✅｜T-20 ✅｜T-21 ✅｜T-22 ✅
（全部經 Opus 驗證）｜T-15 / T-16 / T-17 ⬜ 未開始。**

🎯 **現在該做的：開 Fable 視窗重新規劃**——Phase 1.5 是使用者中途插進來的需求，
做完之後 Phase 1 剩下的 T-15/T-16/T-17 卡片內容還停在「只有照片一種輸入」的假設，
而且這五輪累積了一批需要裁決的事（見下方第 0 節）。**先規劃再開工，不要直接叫
Sonnet 做 T-15。**
...
≤20%，實測 +1.1%）」。**這條裁決會直接影響 T-17 §7-2 的驗收判準怎麼寫**，見第 0 節 B。
...
## 0. 【現在該做的事】開 Fable 視窗重新規劃 — 給 Fable 的交接

**Phase 1.5（T-20/T-21/T-22）已全數通過 Opus 驗證，Phase 1 只剩 T-15/T-16/T-17。**
但這三張卡是 2026-08-16 T-08 細化的，寫的時候還沒有 Phase 1.5、也還沒踩到後面五輪的坑。
**下面 6 件事需要 Fable 裁決或更新卡片，請逐項處理**（重要性由高到低）。

### A. T-15 的卡片範圍與實際需求已經對不上（必處理）

T-15 卡在 2026-08-27 加了一條 Fable 補註：「CLI 要一併收進 `--text` 與 `--scene`
兩種新輸入，三種輸入互斥」。**但卡片的「產出」「執行步驟」「自我檢查」三欄
到現在還是只寫照片鏈**（步驟 1 只有 T-10→T-11→T-12→T-13→T-14）。

Sonnet 是**逐字照做**卡片步驟的角色，補註寫在卡片下方很可能被漏掉。
→ 請把補註落實進步驟與自我檢查（例如：三種輸入各跑一次、同時給兩種要報錯、
`analysis.json` 對三種輸入都要有 `dims_source` 標記）。

### B. T-17 §7-2 的驗收判準必須現在就裁決，不能等到驗收現場（最重要）

SPEC §7-2 寫「生成 IR 的 RT60 **各頻段誤差 < 20%**」。但這五輪已經三次撞到同一件事：

| 出處 | 實測 | 性質 |
|---|---|---|
| 地雷 #14 / T-13 | Sabine 0.348s vs 實測 IR 0.748s（125Hz） | 小房間低頻非擴散場，系統性偏差 |
| T-14 Fable 裁決 | pra ground truth 自己對 Sabine 就 +115% | 判準物理上不可達 → 改機制閉環＋實例閉環 |
| T-21 複驗（2026-08-28） | 臥室 125Hz **+27.5% / +34.8%**、家用小走廊 **+114.4%**、走廊 **+158.6%** | 陡峭頻段階梯的量測混頻偏差，已定位、誠實回報中 |

§7-2 比的是「生成 IR vs **真實 IR 的 T30**」（量測 vs 量測，與比 Sabine 不同類），
但同一個低頻混頻機制仍然會作用。**如果不先裁決，驗收現場只有兩條路：
硬套 20% 全面未達標，或當場放寬判準——後者就是 WORKFLOW §5 明列的紅旗
「把驗收條件改寫成做得到的版本」。** 事前裁決不是放寬，事後改才是。

→ 請決定：§7-2 是否逐頻段一律 20%？低頻（125/250Hz）是否另立判準或另列說明？
判準若要調整，理由與證據鏈要像 T-14 那樣寫進卡片，讓 Opus 驗收時能對抗檢查。

### C. 幾何量程 10m 與引擎 200m 的落差怎麼在驗收裡呈現

T-22 把**引擎**驗證到 200m 級，但 T-11 的**照片幾何**適用範圍仍是 ≤10m
（metric depth 模型量程實證上限 ~20m，體育館實際 150m 只說 3.61m）。
SPEC §7-2 已定案「超範圍場地以手動尺寸（F-09）跑後半段管線，報告逐場地標明尺寸來源」。

→ 但還缺一條規則：**T-17 報告要不要把「自動幾何場地」與「手動尺寸場地」的達標率
分開統計？** 混在一起算，手動尺寸的好成績會稀釋掉自動路徑的真實表現，
而自動路徑才是 F-01 的產品主張。
→ 另外 ROADMAP 記著「換 Metric-Indoor-Large 延後至 T-17 驗收後再評估」，
請確認是否維持（現在 T-22 已把引擎的尺度天花板拆掉，瓶頸只剩幾何這一段）。

### D. 累積的技術債要不要開卡（5 條，都不阻擋，但 T-15 是收斂的最後時機）

| # | 內容 | 出處 |
|---|---|---|
| 1 | `ir_synth.build_pra_materials()` 與 `gen_ir_manual.build_surface_material_dict()` **重複實作**（兩處都還在） | T-14 卡「坑」第 1 條 |
| 2 | 輸出 JSON 的 `warnings` 把**解析 note**（「聲源空間：preset 'bedroom'」）與**真警示**混在同一欄 | T-21 Opus 複驗非阻斷觀察 |
| 3 | `IR_MATCH_WINDOW_RMS_FLOOR_DB = -60.0` 的餘裕只有 ~3dB（崩壞案例實測 −63.2/−63.6dB） | T-22 已知限制，已寫進 config 註解 |
| 4 | `check_audio.py` 不帶參數時 `exit 0`（應為 2）、`test_segmentation.py` 全失敗時 `exit 0`（應為 1） | 地雷 #10，兩處**都還沒修** |
| 5 | T-13 零/負尺寸、T-11 量程規則預設放行 → 兩個「靜默失效」缺口待補防呆 | T-11/T-13 驗證時發現 |

→ 請決定：併進 T-15 卡的執行步驟，還是開一張獨立的小卡（例如 T-18 技術債收斂）。
第 1、2、5 條與 T-15 的整合工作天然重疊，第 3、4 條是獨立的。

### E. 四件「等使用者」的事，其中一件會擋 T-17

- 📷 **9 張照片的來源網址未補**（`assets/SOURCES.md` 仍有 2 處「待補」）——
  **T-04 的自我檢查第 2 項目前是不符合狀態**，會影響 T-17 報告的可追溯性。
  請決定是否列為 T-17 的前置條件。
- 🎤 **真實說話乾聲**（放 `assets/dry/`）——`neighbor_voices` 用合成拍手示範不出「講話聲」
  的感覺，有真實乾聲後重跑 `gen_ir_coupled.py` 會更有說服力。
- 📷 **一張小房間的 360 環景**——環景路徑目前沒有任何「範圍內維持 medium」的防濫殺對照案例。
- ❓ **T-07（Image2Reverb baseline）要不要做**——需使用者授權 clone/下載，限時 2 小時。

### F. ROADMAP 的勾還沒打（只有 Fable 能改）

[ROADMAP.md](ROADMAP.md) 的 Phase 1（T-10~T-14）與 Phase 1.5（T-20/T-21）checkbox
**全部還是空的**，但這些卡都已通過驗證。Sonnet 被禁止改 ROADMAP，所以只能 Fable 順手同步。
Phase 1.5 那節的「⚠️ T-14 目前 🔵 待 Opus 驗證」註記也已過期。

---

### 給 Fable 的背景重點（規劃時別忽略）
...
| T-15 | CLI 整合（照片／文字／複合場景 → IR + 報告） | ⬜ **未開始（下一步，但卡片需 Fable 先更新，見第 0 節 A）** |
| T-16 | 分析視覺化 | ⬜ 未開始 |
| T-17 | MVP 驗收（SPEC §7 四項） | ⬜ 未開始（§7-2 判準需 Fable 先裁決，見第 0 節 B） |
...
【現在該做的 — 開 Fable 視窗重新規劃】（模型選 Fable，另開視窗）
  貼 WORKFLOW.md §2.3 的規劃 Prompt，情況描述用本檔第 0 節的六件事。
  完整可貼版本見第 0 節開頭。

【Fable 規劃完之後】（模型選 Sonnet）
  貼：「執行 TASKS.md 的任務 T-15。先讀 CLAUDE.md 和該任務卡的全部內容再動工，
      完成後執行任務卡裡的自我檢查，最後照 WORKFLOW.md 第 4 節做收工程序。」
```

### ⚠️ 給下一個視窗的提醒

- **先規劃再開工**：T-15/T-16/T-17 的卡片寫於 2026-08-16，那時還沒有 Phase 1.5、
  也還沒踩到後面五輪的坑。直接叫 Sonnet 做 T-15 會做出「只吃照片」的 CLI。
- **T-17 §7-2 的判準要事前裁決**（第 0 節 B）——事前裁決不是放寬，驗收現場才改才是。
- **T-15/T-16 只要動到 IR 生成路徑，就要排使用者試聽關卡**（SPEC §7-4）。
  數字合理 ≠ 聽起來對，本專案已經吃過五次虧。
- 新增 zero-shot 分類的地方必讀地雷 **#13**（softmax 無法表達「以上皆非」）；
  任何「目標 vs 量測」並列的輸出必讀地雷 **#15**。
- T-13 的零/負尺寸、T-11 量程規則的預設放行兩個靜默缺口，T-15 整合時一併補防呆。
