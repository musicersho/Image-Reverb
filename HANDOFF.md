...
**T-10（專案骨架與影像前處理）做完一輪自檢，但 Opus 驗證 🟠 退回**——
一個必修缺陷：equirect 前處理**順序反了**（先裁黑邊、再判環景），會把環景影像的
均勻極點誤判成黑邊裁掉，靜默破壞投影幾何、嚴重時整條環景路徑被跳過。
詳見本檔第 6 節新增的地雷第 11 條；完整技術細節與修法在 [TASKS.md](TASKS.md)
T-10 卡的「Opus 驗證結果」。其餘部分（投影幾何、環景判定依據、黑邊裁切、
錯誤處理、HEIC）**都驗證通過，不用重做**。

**下一步：換 Sonnet 修 T-10 這一個順序缺陷**（不是重做整張卡）。
修好、重新驗證通過後，T-11 與 T-12 可並行。
...
| T-10 | 專案骨架與影像前處理（含環景投影） | 🟠 **退回，Sonnet 下一步修**（順序反了，見地雷 #11） |
| T-11~T-17 | Phase 1 其餘任務（已細化） | ⬜ T-11/T-12 可在 T-10 修好後並行 |
...
11. **🔴 equirect 前處理順序：要先判環景，再決定要不要裁黑邊——順序反了會裁到極點。**
