...
【T-36 通過後 — 回 Fable 做 gate 規則就地定案】（模型選 Fable）
  ...
  規則 1/2/3＋地雷 #23/#24 一次定案（**不得再展延**），並規劃 CLIP 治療卡＋
  評估要不要開陳設公式修正輪。
```

### ⚠️ 給下一個視窗的提醒

- **Phase 1.7／1.8 全輪紅線（共同鐵則 6，兩輪相同）**：gate 判定規則
  （`compute_materials_confidence()` 與 `run_photo()` 的觸發／放行條件）與
  scene_cues 段**一行都不許動**；陳設資料不得餵進任何信心軸——觀測模式下也一樣，
  偵測結果只進報告欄位。gate 規則的定案時點在 T-36 交回時（裁決 T-33-A 裁決 C）。
