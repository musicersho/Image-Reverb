...
> 最後更新：2026-08-28（Fable 規劃視窗：T-21 退回裁決＋T-22 開卡）｜對應 commit：見 `git log --oneline -1`
> **新視窗請先讀 [CLAUDE.md](CLAUDE.md) 知道自己的角色，再讀本檔知道現在的狀況。**
>
> 驗證本檔是否過期：看 [DEV_LOG.md](DEV_LOG.md) 最上面一筆是不是 `2026-08-28 (30)`；
...
**T-10 ✅｜T-11 ✅｜T-12 ✅｜T-13 ✅｜T-14 ✅（Opus 驗證 2026-08-28）｜
T-20 ✅（Opus 驗證 2026-08-28）｜T-21 🟠 退回（2026-08-28）｜T-22 ⬜ 新卡。**

⚠️ **T-21 退回原因（一句話）**：交付的巨蛋場景（160×130×45）觸發 T-14 引擎的
固定早期窗尺度上限，聲源空間 IR 的 2k/4k 晚期殘響 −94% 且全程無警示
（本專案頭號失敗型態「安靜地輸出看似合理的錯誤結果」再現）。
Fable 已裁決**修引擎不縮場景**（理由：T-20 的 `stadium_dome` preset 200×160×55
也在失效區、巨蛋是 F-17 原始需求場景）——完整裁決在 TASKS.md T-21 卡與 DEV_LOG (30)。

🎯 **現在該做的：開 Sonnet 視窗執行 T-22（引擎尺度自適應，TASKS.md 有完整卡片）
→ 接著 T-21 修正輪（卡上有步驟）→ 重生 v4 試聽檔請使用者重聽一次
（v3「確認OK」是對有缺陷的 IR 給的，不能沿用）→ Opus 依序複驗 T-22、T-21。**
使用者試聽紀錄：T-14 ✅「目前聽起來 OK」｜T-20 ✅「沒有問題」｜
T-21 v3「確認OK」（三輪迭代）——**但 v3 的 OK 是對高頻晚期殘響缺失的 IR 給的，
T-22 修好後必須重聽 v4，不能沿用**。人耳回饋全文與修正記在 TASKS.md T-21 卡。
...
## 0. 【現在該做的事】開 Sonnet 視窗執行 T-22

（Opus 驗證輪已於 2026-08-28 完成：T-14 ✅、T-20 ✅、T-21 🟠 退回。）

**下一步：開 Sonnet 視窗執行 T-22**（T-14 引擎尺度自適應）。
貼：「你是執行者。請讀 TASKS.md 的 T-22 任務卡，逐字照做。」

> **給 Sonnet 的重點提醒**：
> 1. T-22 動的是**已通過驗證的 T-14 檔案**，所以卡上的「零回歸」判準是硬的：
>    小/中房間兩條 IR 必須與交付版 **bit-identical**（MD5 相同），既有 11 項迴歸
>    判準一個都不准放寬。做不到就 🔴 卡關回報，不要硬改。
> 2. T-22 之後接 T-21 修正輪（T-21 卡上有完整步驟），兩者做完先重生 v4 試聽檔
>    **請使用者重聽**再進 Opus 複驗——v3 的「確認OK」不能沿用。
> 3. `source .venv/bin/activate` 後 `python scripts/test_ir_synth.py` 可隨時重跑迴歸。
...
| T-14 | IR 合成引擎 v1（早期 image-source + 晚期 shaped-noise） | ✅ **通過**（Opus 驗證 2026-08-28）｜⚠️ 固定早期窗有 80–100m 尺度上限，T-22 修正中 |
| T-20 | 文字場景描述 → IR（13 preset＋解析器，F-16） | ✅ **通過**（Opus 驗證 2026-08-28）｜⚠️ `stadium_dome` preset 受 T-14 尺度上限影響，T-22 修好即解 |
| T-21 | 複合場景引擎 v1（路徑串接＋TL 表，F-17） | 🟠 **退回**（Opus 2026-08-28：巨蛋 2k/4k −94% 靜默錯誤）→ 等 T-22 後跑修正輪＋使用者重聽 |
| T-22 | T-14 引擎尺度自適應（早期窗/匹配窗隨尺寸調整） | ⬜ **未開始（下一步）**——Fable 裁決 2026-08-28 開卡，Sonnet 執行 |
| T-15~T-17 | Phase 1 其餘任務（已細化） | ⬜ 未開始（T-15 等 T-21 複驗通過後整合三種輸入） |
