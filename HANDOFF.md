...
> ## 🔍 2026-09-22 Opus：T-63 r2 審查完成——結論「**需 Fable 再修（r3）**」。**現在該做的是：開 Fable 新視窗（WORKFLOW §2.3），請它依 T-63 卡末「🔍 Opus 審查 r2」段修出 r3，並順手修 T-64 的一行類別字串（G-29）**；r3 再交 Opus 審 diff，之後才輪到您核准。T-70 步驟 3a（下載）**仍可以先跑**（一次只開一個視窗）
>
> - 合規面沒問題：門檻數字（≥4/5、<20%、名單 ≥3、≤10 m）一個都沒動、T-17／T-17-R2 兩個 FAIL 保留；您的兩個決定（BUT 材質文字可照抄、R112 取外接盒）都沒有把標準放寬，也寫成了照規則就能執行的條文。r1→r2 改了哪幾行，Opus 自己逐行比對過，跟 Fable 列的一致。
> - **必修兩處（白話）**：① 公開資料集的參考 IR 如果存成「很多位置×很多麥克風」的大陣列，r2 沒說怎麼切成一支一支（上一輪要求的「單一量測位置」漏寫了）——要加「切不成單支就停下來」；② 某房間 20 支參考 IR 裡若有 1 支算不出數字，是整間房出名單、還是丟掉那 1 支，r2 沒講——這一步發生在參數已鎖定之後，留著讓人選就等於「看到走向再決定名單」，要事先寫死。
> - **要先處理的接縫**：T-64（合成圖分組）規則寫「客廳／臥室」，但素材表的實際類別字是「客廳臥室」（沒有斜線）；照規則用程式分組會把 2 張小房間誤分到開發集。**T-64 開跑前請 Fable 先修這一行。**
> - 其餘是可以一起修的小處（重跑紀錄放哪、複驗要固定套件版本、官方材質文字要多擋「吸音係數」類欄位、照片來源範圍對齊、「<20%」與舊程式「≤20%」的邊界等），逐條在 T-63 卡 G-1～G-33。Fable 補列的 `MemoryError` 可以保留。
> - T-70 步驟 3a（下載＋算指紋）先跑，跟封存鐵則不衝突；只建議 Fable 先補一句「9.3 GB 的壓縮檔先別刪」（不補照跑也不算違規）。
> - 本視窗只追加 T-63 卡末、DEV_LOG、TODO、HANDOFF；T-63 四軸不變；SPEC／WORKFLOW／src／scripts／data／output／assets 零 diff。本審查不構成「r2 審過」，鐵則 18-a 繼續從嚴（沒有任何授權）。
> - **📌 給所有視窗（含 Codex）——兩條新鐵則（TASKS.md Phase 1.10；請之後在頂端加新段的視窗把本條原樣保留，直到 Fable 寫下 R3 解除紀錄）**：
...
>   **(18-a)** 鐵則 18 同樣適用 `assets/photos_synth_heldout/` 與 `assets/reference_rooms_heldout/`；T-17-R3 受驗 run 是唯一授權；Opus 審過 T-63 r(n) 之前從嚴＝沒有任何授權（r1 已審、未過）。

> ## 🔮 2026-09-21 Fable：T-63「判準 v2 草案 修訂版（r2）」已起草——**現在該做的是：開 Opus 新視窗，逐字貼下面的 Prompt 審 r2 的 diff**；Opus 結論「可送使用者核准」後，您回 Fable 一句「T-63 r2 核准」（或寫要改哪裡）。**T-70 步驟 3a（下載）現在就可以跑，不必等 Opus**（一次只開一個視窗）
>
> - **要貼給 Opus 的 Prompt（逐字）**：
>   　你是驗證者，只審查不修改。請審 TASKS.md T-63 卡交接筆記末段的「判準 v2 草案 修訂版（r2）」（R2-0～R2-5；R2-2＝完整合併條文、R2-2b＝WORKFLOW §5.4.4 註記條文段、R2-2c＝鐵則 18-a(r2) 條文段）。先讀 CLAUDE.md、HANDOFF.md 頂端、T-63 卡全文（原草案＋你兩輪的審查意見＋r1＋r2）、Phase 1.10 鐵則 18／18-a／18-a（r2 指標）／19 與「關鍵路徑更新 r2」、T-64／T-65／T-70 三張卡的「r2」段、T-17-R3 保留號的 r2 補、`assets/reference_rooms_heldout/SURVEY.md`。
>   　審查項目：① 對照 R2-4 回應表，你上一輪的六處〔擋〕（B-6、C-1、C-2、D-1、D-2、D-3(i)～(v)）與其餘〔改〕（A-3、B-4、B-5、C-3～C-10）是否都已處理、條文裡找得到、沒有「表上說採納、條文沒有」；② r2 只需審 diff——R2-2 內不含「〔r2：」的行應與 r1 R1-2 逐字相同（請自己核對；被改寫的 10 行列在 R2-4 末的「diff 索引」），含「〔r2：」的行逐行審；③ WORKFLOW §7 合規：門檻數字（≥4/5、<20%、名單 ≥3、≤10 m）有沒有被動到、舊 verdict 是否保留；特別審使用者的兩個決定（R2-0：BUT 官方材質文字可照抄＝白名單抽取；**R112 留在名單、取外接盒——與你和 Fable 的建議不同**）寫成的條文是否可機械執行、有無放寬；
>   　④ 新條文的可機械驗證性與漏洞：定義 RR 五條（含 Fable 補列的 `MemoryError`，可要求刪除）、P-1 必含清單與漏鎖處置、P-5 的 `find` 檢查、V2-2(a)(iii) 照片合格三條件、V2-2(d) 的「支」定義／三道過濾／讀取方式／「停，回 Fable」條款、18-a(r2)(2)④ 複驗授權、T-70 r2-3b 的欄位白名單與黑名單；⑤ R2-2b 的 §5.4.4 十點讀法有沒有把任何一點讀鬆；⑥ Fable 的決定「T-70 步驟 3a 不必等你審完就可以下載」（T-63 R2-3 F-7、T-70 r2-0／r2-3a）是否與鐵則 18／18-a 從嚴相容；⑦ T-64／T-65／T-70／T-17-R3 與 r2 的接縫是否還有矛盾。
>   　輸出逐條意見與總結（「可送使用者核准」或「需 Fable 再修（r3）」），只追加在 T-63 卡末，不要改其他任何檔案的既有文字；T-63 四軸維持不變；SPEC §7 與 WORKFLOW 不得動。最後照 WORKFLOW 第 4 節收工。
> - **您這一輪的兩個決定（原話與選項全文在 T-63 卡 R2-0）**：① BUT 官方說明檔的材質文字 →「**可以，只抽材質欄給我看**」；② R112（L 形旅館房間）→「**留著，取兩個方塊的外接盒**」。第 ② 題和 Fable／Opus 的建議不同，Fable 已照您的決定寫成條文（外接盒只能由官方數字機械算出，算不出唯一值就停下來再問您）；**您在核准 r2 之前隨時可以改回「移出名單」**。
> - **r2 改了什麼（白話）**：① 「環境故障可以重跑」現在要同時符合五條（沒留下任何輸出檔、只認被系統中止或磁碟滿／記憶體不足這幾種、程式碼一個字都沒變、同一份參數檔、最多重跑 2 次），封存規則也用同一條；② 參數檔漏放進鎖定 commit＝該場地直接算沒過，不能拿來把名單變小；③ T-65 做完請您試用時改用合成開發集的照片，健身房和其他要驗收的場地在正式鎖定前一律不給您看參數、不給您聽；④ 複驗者可以把參考值重算一次、結果要一模一樣；
>   ⑤ 公開資料集的參考 IR 怎麼挑已寫死（只要空房、不要藏在抽屜裡的麥克風、實驗室只用沒掛布幔的那種配置、每間最多 20 支）；照片要長邊 ≥1024 px 而且看得到地板和一面牆；⑥ WORKFLOW §5.4.4 要加的註記全文已寫在 T-63 卡（R2-2b），跟判準一起送審，本輪沒有動 WORKFLOW；⑦ 其餘是如實揭露的句子（盲聽不檢驗照片分析、參考值本身有誤差、「可算性檢查」不是品質檢查、域外標 attention 那條有 5 張等於自動過）。**門檻數字（≥4/5、<20%、名單 ≥3、≤10 m）一個都沒動。**
> - **名單數字（白話；T-63 卡 R2-3 F-4）**：最多 7 間（健身房＋SoundCam 2＋BUT 4）。**現在真正確定的只有健身房 1 間**——其餘 6 間都要等下載後檢查照片（SoundCam 的照片是論文插圖，很可能不到 1024 px）。名單要 ≥3 間才有得判＝待查 6 間至少要過 2 間；最壞情況只剩健身房 → 判準 2「樣本不足」→ R3 必 FAIL。反方向：名單每一間都要過，R112 用外接盒會偏長，很可能單獨讓判準 2 不過。另外提醒：R3 時最多有 19 份參數檔要您逐份確認（多數可照抄設計表或官方文字）。
> - **T-70 拆成 3a／3b**：3a＝下載＋算檔案指紋＋列目錄結構（**現在可跑**：開新 Sonnet 視窗貼 WORKFLOW §2.1，`T-XX` 換成 `T-70`，並加一句「從『🔮 Fable 修訂 r2』的步驟 3a 開始，只做 3a」）；3b＝依核准後的規則挑參考 IR、寫名單（要等 T-63 的 `criteria:` commit）。建議順序：Opus 審 r2 →（或先）T-70 3a → T-67 → **T-64 路徑 S（提前）** → T-66 → T-65 → criteria commit 後 T-70 3b。
> - **卡片變動**：T-63 追加 r2 指標行＋R2-0～R2-5（四軸不變；未核准不生效）｜Phase 1.10 追加「18-a（r2 指標）」與「關鍵路徑更新 r2」｜T-64 追加 r2-1～r2-3｜T-65 追加 r2-1～r2-3（試用素材、失敗不留檔、`--params --check`）｜T-70 追加狀態補行＋r2（3a／3b）｜T-17-R3 保留號追加 r2 補。TASKS.md 純新增行（零刪除行）；SPEC／WORKFLOW／src／scripts／data／output 零 diff。詳見 DEV_LOG `2026-09-21 (180)`。
> - **📌 給所有視窗（含 Codex）——兩條新鐵則（TASKS.md Phase 1.10；請之後在頂端加新段的視窗把本條原樣保留，直到 Fable 寫下 R3 解除紀錄）**：
>   **(18)** `assets/photos_real_heldout/`（T-64 就位後）內的檔不得以任何模型或分析／評測腳本處理，**沒有例外**（全套測試也不得掃到）；只准算 sha256、讀尺寸／EXIF、人眼看；不得複製、不得一批兩用。
>   **(19)** `output/mvp_acceptance_r2/` 與 R2 本輪 18 個 `output/<run>/` 目錄唯讀，任何重跑寫新目錄（T-67 完成前，**不要**對 `scripts/t17r2_*.py` 下任何引數——它們不解析引數、會直接執行並覆寫）。
>   **(18-a)** 鐵則 18 同樣適用 `assets/photos_synth_heldout/` 與 `assets/reference_rooms_heldout/`；T-17-R3 受驗 run 是唯一授權；Opus 審過 T-63 r(n) 之前從嚴＝沒有任何授權（r1 已審、未過）。

> ## 🔍 2026-09-21 Opus：T-63 r1 審查完成——結論「**需 Fable 再修（r2）**」。**現在該做的是：開 Fable 新視窗（WORKFLOW §2.3），請它依 T-63 卡末「🔍 Opus 審查 r1」段修出 r2，並向您問兩個〔決〕；r2 再交 Opus 審 diff，之後才輪到您核准**。T-70 步驟 3（下載）**繼續暫停**，要等 r2 審過
>
> - 合規面沒問題：門檻數字一個都沒放寬、T-17／T-17-R2 兩個 FAIL 保留、原域內失敗（健身房）仍在名單且逐場地揭露；Fable 對上一輪意見逐點都有回應。
> - **必修六處（白話）**：① WORKFLOW §5.4.4 要加的註記還沒寫出文字，沒寫出來就沒辦法審；② 「環境故障可以重跑」的例外沒鎖住程式碼，可能被拿來「修 bug 再重跑」；③ 參數檔沒放進鎖定 commit 的情況沒有規定算什麼（要算未達）；④ T-65 做完後請您試用時，用的是健身房照片——等於在正式鎖定前預演健身房的材質確認，要改用開發集的合成圖；⑤ 封存規則（18-a）沒有允許複驗者重算，跟「複驗通過才解除」自相矛盾；⑥ 公開資料集的參考 IR 要先寫死挑哪些（空房、非藏起來的麥克風、實驗室選哪一種配置），BUT 的 R112 是 L 形房間、尺寸不唯一。
> - **要您決定的兩題（Fable 會問）**：BUT 官方檔裡的材質文字能不能讓您照抄（建議可以，由程式只抽材質欄給您看）；R112 要移出名單，還是取兩個方塊的外接盒（建議移出）。
> - 誠實提醒：參考值「可算性檢查」幾乎一定會過（`ir_metrics.py` 沒有雜訊判斷）；「域外要標 attention」這條在 9 張受測照片中有 5 張是自動成立。兩者都只要求如實寫進報告，不加門檻。
> - 本視窗只追加 T-63 卡末、DEV_LOG、TODO、HANDOFF；T-63 四軸不變；SPEC／WORKFLOW／src／scripts／data／output 零 diff。鐵則 18-a 繼續從嚴（沒有任何授權）。
> - **📌 給所有視窗（含 Codex）——兩條新鐵則（TASKS.md Phase 1.10；請之後在頂端加新段的視窗把本條原樣保留，直到 Fable 寫下 R3 解除紀錄）**：
>   **(18)** `assets/photos_real_heldout/`（T-64 就位後）內的檔不得以任何模型或分析／評測腳本處理，**沒有例外**（全套測試也不得掃到）；只准算 sha256、讀尺寸／EXIF、人眼看；不得複製、不得一批兩用。
>   **(19)** `output/mvp_acceptance_r2/` 與 R2 本輪 18 個 `output/<run>/` 目錄唯讀，任何重跑寫新目錄（T-67 完成前，**不要**對 `scripts/t17r2_*.py` 下任何引數——它們不解析引數、會直接執行並覆寫）。
>   **(18-a)** 鐵則 18 同樣適用 `assets/photos_synth_heldout/` 與 `assets/reference_rooms_heldout/`；T-17-R3 受驗 run 是唯一授權；Opus 審過 T-63 r(n) 之前從嚴＝沒有任何授權（r1 已審、未過）。

> ## 🟡 2026-09-21 Sonnet：T-70 步驟 0～2（調查）完成，**使用者已授權下載（SoundCam 兩房間＋BUT 四房間；原話在 T-70 卡交接筆記第 10 點）——但步驟 3 尚未開跑：要等 Opus 審完 T-63 r1（V2-2(a)）**。**現在該做的是：先開 Opus 審 T-63 r1（Prompt 見下方 Fable 段）→ 通過後開新 Sonnet 視窗貼 §2.1（`T-XX`＝`T-70`）從步驟 3 開始**
>
> - 四軸：**工程：進行中（停點；步驟 3～4 未開始）｜實驗：不適用｜產品：不適用｜MVP：不適用**。調查表：`assets/reference_rooms_heldout/SURVEY.md`；完整交接筆記在 TASKS.md T-70 卡末。
> - **五條件全符合的房間**：官方文件即可確認＝**2 個**（SoundCam：Treated Room 實驗室約 4.9×5.1×2.7 m、Conference Room 會議室約 6.7×3.3×2.7 m；MIT 授權）；**只差照片、要下載才能確認＝4 個**（BUT ReverbDB 的 L207、L212、R112、C236；CC BY 4.0；整包 9.3 GB、且 IR 是 16 kHz）。連同 `mit_gym` 名單 3～7 個；T-63 F-4「少於 2 個」預告暫不成立，但下限（3）脆弱。
> - 不入清單：AIR（尺寸查不到＋假人頭雙耳）、ACE（需註冊）、dEchorate（單一房間、11 種牆面配置）、OpenAIR（網站停權）、SoundCam 客廳（無尺寸）。
> - 步驟 3 前置：使用者授權下載範圍 ＋ T-63 r1 V2-2(a) 經 Opus 審過。交 Fable／Opus 的風險備註在 T-70 卡交接筆記第 5 點（參考 IR 限空房／非 boxed 麥克風、16 kHz、R112 非方塊形、SoundCam「全向」待核）。
> - 這一段整段沒有殘響時間數值——**請後續視窗也不要把資料集論文的殘響時間表抄進任何檔案（程序 P-5）**。
> - **📌 給所有視窗（含 Codex）——兩條新鐵則（TASKS.md Phase 1.10；請之後在頂端加新段的視窗把本條原樣保留，直到 Fable 寫下 R3 解除紀錄）**：
>   **(18)** `assets/photos_real_heldout/`（T-64 就位後）內的檔不得以任何模型或分析／評測腳本處理，**沒有例外**（全套測試也不得掃到）；只准算 sha256、讀尺寸／EXIF、人眼看；不得複製、不得一批兩用。
>   **(19)** `output/mvp_acceptance_r2/` 與 R2 本輪 18 個 `output/<run>/` 目錄唯讀，任何重跑寫新目錄（T-67 完成前，**不要**對 `scripts/t17r2_*.py` 下任何引數——它們不解析引數、會直接執行並覆寫）。
>   **(18-a)** 鐵則 18 同樣適用 `assets/photos_synth_heldout/` 與 `assets/reference_rooms_heldout/`；T-17-R3 受驗 run 是唯一授權；Opus 審過 T-63 r1 之前從嚴＝沒有任何授權。

> ## 🔮 2026-09-21 Fable：T-63「判準 v2 草案 修訂版（r1）」已起草——**現在該做的是：開 Opus 新視窗，逐字貼下面的 Prompt 審 r1 的 diff**；審過後您回 Fable 一句「T-63 r1 核准」（或寫要改哪裡）。其餘可交錯進行：Sonnet 跑 T-70（只到步驟 2 調查）、T-67 → T-66 → T-65、T-64 路徑 S
>
> - **要貼給 Opus 的 Prompt（逐字）**：
>   　你是驗證者，只審查不修改。請審 TASKS.md T-63 卡交接筆記末段的「判準 v2 草案 修訂版（r1）」（R1-0～R1-5）。先讀 CLAUDE.md、HANDOFF.md 頂端、T-63 卡全文（原草案＋你上一輪的審查意見＋r1）、Phase 1.10 鐵則 18／18-a／19、T-64 卡「路徑 S 修訂」、T-65 卡「規格追加 r1」、T-70 卡。
>   　審查項目：① r1 是否逐點回應了上一輪意見（對照 R1-4 回應表；不採納的 ②-8、部分採納的 ①-3(b) 理由是否成立）；② WORKFLOW §7 合規（原 verdict 保留、原域內失敗不得消失、門檻數字未在看過結果後放寬——特別審 R1-3 的 F-1～F-5：held-out 改 AI 合成圖、手機錄音收窄為報告項、WORKFLOW §5.4.4 須同步加註）；③ 程序 P、V2-2(a)～(j)、V2-5(a)～(c)、鐵則 18-a 是否可機械驗證、有無可鑽的漏洞（含 P-4 的「環境故障同 sha256 重跑」例外）；④ T-64／T-65／T-70 與 r1 的接縫是否還有矛盾。
>   　輸出逐條意見與總結（「可送使用者核准」或「需 Fable 再修（r2）」），只追加在 T-63 卡末，不要改其他任何檔案的既有文字；T-63 四軸維持不變；SPEC §7 不得動。最後照 WORKFLOW 第 4 節收工。
> - **您這一輪的決定（原話與選項全文在 T-63 卡 R1-0）**：U1「目前無法實際到各個場地做此動作，請提供替代方案」→ 選「改用公開量測資料集」；T-64 可行範圍 →「都沒辦法，想用合成圖」；U2「留在名單，參數事前寫死」；U3「報告項，強制揭露」；U4「小房間全進 held-out」。
> - **r1 改了什麼（白話）**：① 參數要先存檔鎖定（commit）才准出聲音，鎖定後改檔就算答錯——堵住「先聽再調參數」；② 盲聽用的 held-out 改成 AI 合成圖（Codex 今天生的十張，依檔案指紋分組封存），報告必須寫「不能外推到真實照片」，PASS 字樣要帶「held-out＝AI 合成圖」；
>   ③ RT60 <20% 的判定名單＝健身房（尺寸寫死 9×6×2.9、材質您只看照片確認）＋**公開資料集的小房間**（新卡 **T-70**：先調查、停下來請您授權下載、下載後立刻封存）；少於 3 個場地仍記「樣本不足」；④ 壁球場＝強制揭露的報告項；⑤ 安全判準改成可用指令檢查的寫法。**門檻數字（≥4/5、<20%）一個都沒動。**
> - **請您核准時留意一點**：我在選項說明寫過「您若哪天在家方便錄，通過品質檢查的也可以加入」，r1 把它收窄為「只當報告項、不入判定名單」（只會更嚴、條文單純很多）。要恢復請在核准前告訴 Fable。
> - **風險預告**：T-70 查證後，符合條件（≤10 m、有官方照片、有官方尺寸、有專業量測 IR、授權可用）的公開房間若不到 2 個，判準 2 仍會樣本不足、R3 仍必 FAIL——T-70 步驟 2 會把實際數字給您，屆時回 Fable 決定。
> - **卡片變動**：T-63 追加 r1（原草案與 Opus 意見原字保留；四軸不變；未核准不生效）｜T-64 追加「路徑 S 修訂」（真實素材軌擱置）｜T-65 追加 r1-1～r1-5（`--suggest` 既有目標拒寫、`--out-dir`、實跑一律導 scratchpad 以免寫進 R2 唯讀的 `output/site_photo_gym/`；`surfaces.<面>.attention_reasons[]`；測試補 V2-5）｜新卡 T-70｜T-68 資料集 (c) 不納入｜T-69／T-17-R3 開卡條件補註。
>   **新增鐵則 18-a**（TASKS.md Phase 1.10 鐵則段）：鐵則 18 的範圍擴到 `assets/photos_synth_heldout/` 與 `assets/reference_rooms_heldout/`；R3 受驗 run 是唯一授權（三種程式各一次、寫 `output/mvp_acceptance_r3/`）；**Opus 審過前從嚴＝沒有任何授權**。
> - **📌 給所有視窗（含 Codex）——兩條新鐵則（TASKS.md Phase 1.10；請之後在頂端加新段的視窗把本條原樣保留，直到 Fable 寫下 R3 解除紀錄）**：
>   **(18)** `assets/photos_real_heldout/`（T-64 就位後）內的檔不得以任何模型或分析／評測腳本處理，**沒有例外**（全套測試也不得掃到）；只准算 sha256、讀尺寸／EXIF、人眼看；不得複製、不得一批兩用。
>   **(19)** `output/mvp_acceptance_r2/` 與 R2 本輪 18 個 `output/<run>/` 目錄唯讀，任何重跑寫新目錄（T-67 完成前，**不要**對 `scripts/t17r2_*.py` 下任何引數——它們不解析引數、會直接執行並覆寫）。
> - 本輪 Fable 零改動 `src`／`scripts`／`data`／`output`／SPEC／WORKFLOW；TASKS.md 純新增行。動了 TASKS／DEV_LOG／TODO／HANDOFF／ROADMAP／`assets/incoming_real/README.md`（加一行擱置註記）。詳見 DEV_LOG `2026-09-21 (177)`。

> **2026-09-21 Codex 補充：使用者要求的十個合成空間已生成。** 照片、設計尺寸、六面材質與總覽在 `assets/incoming_synthetic_20260921/`；圖留本機。這不是 T-64 真實素材，沒有量測 GT、沒有分組，T-64 狀態不變；以下 Fable 排程與鐵則 18／19 繼續有效。合成素材交付四軸：工程完成（自檢，待獨立審查）／實驗不適用／產品不適用／MVP 不適用。


> ## 🔍 2026-09-21 Opus：T-63 判準 v2 草案條文審查完成——結論「**需 Fable 修改後再審**」。**現在該做的是：開 Fable 新視窗（WORKFLOW §2.3），請它依 T-63 卡交接筆記的審查意見修稿，並向您問 U1～U4 四個決定**；其餘關鍵路徑（T-67 → T-66 → T-65、T-64 拍照）照舊
>
> - 門檻數字**沒有被放寬**、T-17／T-17-R2 兩個 FAIL 保留、沒有自改自批——方向合規。但有五處會被鑽或會卡住 R3：① 參數確認與試聽先後沒鎖（可先聽再改參數）；② `mit_gym` 的參數不是使用者實測、參考答案已公開；③ 判準 5(c)「域外」若由 gate 自己判＝自我指涉；④ 鐵則 18 字面上連 R3 本身都禁止；⑤ T-65 `--suggest` 預設會寫進 R2 唯讀目錄 `output/site_photo_gym/`。另有 racquetball 必測反例在 v2 消失、T20／T30 口徑、車內歸屬、T-63／T-64 名單範圍不一致等修正。
> - **U1～U4（要您決定）**：手機錄音能不能當真實參考／健身房要不要留在名單／壁球場反例是門檻還是報告項／T-64 分組要不要讓 ≤10 m 空間多進 held-out（**這題要在 T-64 分組前決定**）。建議 Fable 修稿時一次問。
> - 本視窗只追加 T-63 交接筆記、DEV_LOG、TODO、HANDOFF；T-63 四軸不變；SPEC／WORKFLOW／src／scripts／data／output 零 diff。
> - **📌 給所有視窗（含 Codex）——兩條新鐵則（TASKS.md Phase 1.10；請之後在頂端加新段的視窗把本條原樣保留，直到 Fable 寫下 R3 解除紀錄）**：
>   **(18)** `assets/photos_real_heldout/`（T-64 就位後）內的檔不得以任何模型或分析／評測腳本處理，**沒有例外**（全套測試也不得掃到）；只准算 sha256、讀尺寸／EXIF、人眼看；不得複製、不得一批兩用。
>   **(19)** `output/mvp_acceptance_r2/` 與 R2 本輪 18 個 `output/<run>/` 目錄唯讀，任何重跑寫新目錄（T-67 完成前，**不要**對 `scripts/t17r2_*.py` 下任何引數——它們不解析引數、會直接執行並覆寫）。

> ## 🔮 2026-09-21 Fable：共用圖禁用令**已解除**＋R2 收尾複評（裁決 T-17-R2-A）——產品方向改為「**參數為主、照片為輔**」（SPEC v0.5）。**現在該做的是（一次只開一個視窗）：① Sonnet 跑 T-67 → Opus 驗 → ② Sonnet 跑 T-66 → Opus 驗 → ③ Sonnet 跑 T-65 → Opus 驗；使用者這邊：拍照（T-64）＋找空檔請 Opus 審 T-63 條文**
>
> - **每一步怎麼貼**：Sonnet 用 WORKFLOW §2.1、Opus 用 §2.2，`T-XX` 換成卡號。T-63 的 Opus 審條文 Prompt 在 T-63 卡「怎麼核准」；審完您回 Fable 一句「T-63 核准」或要改哪裡。拍照說明在 `assets/incoming_real/README.md`（照片預設不上傳 GitHub），放好後開 Sonnet 貼 T-64。
> - **禁用令**：已於 `121697a` 解除（紀錄在 TASKS.md T-04 卡裁定 T-04-R §2 第 4 點下方，commit `2b88d89`）。下方各段原有的 📌 禁用令條已由 Fable 移除（原文見 T-04 卡條文與 `git show 121697a:HANDOFF.md`）；下方舊段裡「禁用令尚未解除」等字句是當時的紀錄，以本段為準。
>   R2 held-out 五張＋`t04_gpt_*` 複本現在是一般開發素材，但**不再具 held-out 資格**；檔案不搬不改名、`assets/photos_legacy_20260920/` 不刪、`ground_truth_heldout.json` 的 `dims_m` 不得當幾何 GT。
> - **📌 給所有視窗（含 Codex）——兩條新鐵則（TASKS.md Phase 1.10；請之後在頂端加新段的視窗把本條原樣保留，直到 Fable 寫下 R3 解除紀錄）**：
>   **(18)** `assets/photos_real_heldout/`（T-64 就位後）內的檔不得以任何模型或分析／評測腳本處理，**沒有例外**（全套測試也不得掃到）；只准算 sha256、讀尺寸／EXIF、人眼看；不得複製、不得一批兩用。
>   **(19)** `output/mvp_acceptance_r2/` 與 R2 本輪 18 個 `output/<run>/` 目錄唯讀，任何重跑寫新目錄（T-67 完成前，**不要**對 `scripts/t17r2_*.py` 下任何引數——它們不解析引數、會直接執行並覆寫）。
> - **使用者本輪兩個決定（原話＝選項文字，全文在裁決 T-17-R2-A §0）**：「參數為主、照片為輔」「願意自拍＋量尺寸」。
> - **裁決摘要**：MINC/DMS＝開可行性量測卡 **T-68**（只量不改；步驟 0 會停下來請您授權下載）；陳設公式修正輪＝**不做**（R3 後依事前寫死的條件再議）；`IR_RT60_BASIS` 維持 sabine；gate 不調；**T-59 正式開卡**（改善軌）；幾何 v2＝保留號 **T-69**（等 T-64 開發集）；**T-17-R3** 保留號（條件：T-63 核准＋T-64～T-67 全 ✅）。
>   七個問題的逐項診斷（健身房被擋、材質 8/10 錯、尺寸偏大與 1.1547、尾巴偏長／音樂廳太短、百貨缺幾何出口、工具 `--help`、T-59）在 TASKS.md「Phase 1.10」§1 的表。
> - **SPEC v0.5**：只改定位／流程／F-09／新增 F-18／F-23 註記／§8 一列風險；**§7 驗收判準一字未動**——T-63 草案經獨立 Opus 審＋您核准＋獨立 `criteria:` commit 才生效。T-17 FAIL、T-17-R2 FAIL（R2）永久保留。
> - **誠實預告**：輔助路徑不保證 R3 會 PASS；「尺寸＋材質都由使用者確認」下引擎準不準從沒量過，T-64 的現場拍手錄音是補 in-domain 對照的唯一來源，請盡量錄。
> - 本輪 Fable 零改動 `src`／`scripts`／`data`／`output`／WORKFLOW；動了 TASKS／SPEC／ROADMAP／DEV_LOG／HANDOFF／TODO／`.gitignore`（只加隱私規則）／`assets/` 三份文件＋新增 `assets/incoming_real/` 兩份說明。詳見 DEV_LOG `2026-09-21 (175)`。
...
>   〔2026-09-21 補（另起新行；上一條原文不動）：使用者事後明示「前面沒有回覆的部分視爲認同判斷」——上述解讀成立，Opus **不需**再請使用者補回「GT 確認」；JSON 不需改動。原話逐字在 TASKS.md T-60 卡交接筆記「使用者第二次回覆」。〕
...
> - Opus 全部自己重跑：範圍恰 5 個腳本檔、零 diff 清單全中；鐵則 16(b) 行層級檢查通過；**R1 突變自己重做得到相同判定**（只有 `(a-3)` ❌／`EXIT=1`，還原後 74 條全 ✅）；
>   **鐵則 5 第一類自建舊碼 worktree 實測 36 ❌／38 ✅**；裁定 T-57-D 口徑以自製 items 手算逐項複核相符；R5 用步驟 2(e) 逐字樣板實測（`Traceback＋exit=1 → 1`，舊碼給 0）；R4／R6／第 8 條全部實跑通過；六條 IR MD5 全中；`output/` 不變量成立。
> - 「Opus 驗證重點」13 項紅旗**全部「無」**；「不得回歸」12 項**全部維持**；交接筆記符合鐵則 16／17（30 組 BEGIN/END 配對、結論都在輸出之後、固定 commit 快照、MD5 完整 32 碼且用 `md5 -q`）。
> - **⚠️ 要處理 1（T-04 的事，不是 T-57）**：在當下 `main` 跑 22 支測試，`test_depth.py` 與 `test_pipeline_dedup.py` **EXIT=1**——因為 T-04 替換了 `assets/photos/`（缺 `bathroom_tiled.png` 等 6 項；新圖讓 `test_depth.py` 丟 `TypeError`）。在 `14fc4ac` 的 worktree 上 22 支全 `EXIT=0`。**請 T-04 視窗（或 Fable）處理。**
> - **⚠️ 要處理 2（使用者）**：背景調查 `task_14c97967` 前提已二度證實為誤報，Opus 無控制權，請在 App 背景任務清單關閉；若它已有任何結論或 commit，一律不採用。
> - **🟡 Opus 建議（不擋本卡，交 Fable 排後續）**：(k) 段 fixture 的來源值只有 `clip`／`無` 且全 in-domain，`source=="default"` 與 out-domain 的排除型突變能無聲通過全部測試（程式正確、卡片字面驗收亦達成）；
>   `t17r2_blind_test.run()` 預設 `photos_dir` 綁真實 `REPO_ROOT`；`_cli_outside()` 護欄在 R2 步驟 1 後變恆真；建議輕量 §7 改寫「測試全在系統暫存目錄」。五項詳見 TASKS.md T-57-F1 卡末「✅ Opus 驗證紀錄」§15。
> - T-17-R2 仍等使用者 held-out 照片（Codex 的合成候選 1448×1086 未達 ≥1920px，且已成 T-04 共用開發素材，不算正式 held-out）。詳見 TASKS.md T-57-F1 卡末驗證紀錄與 DEV_LOG `2026-09-20 (166)`。

> 2026-09-20 Codex 全專案完工差距盤點：見 `PROJECT_REVIEW_20260920.md`。核心 CLI 已完成，MVP 首驗 FAIL／R2 待重驗，Phase 2 外掛未開發。新發現 T-04 換圖後 legacy 五張預設路徑全部不存在（備份齊全），需在 R2 前整理；正式 held-out 尚缺、T-57-F1 待審。本次 R2 工具自測全部通過，不取代正式審查。 未跑影像／IR；未改碼與門檻。
...
> - **R1～R8 全部修正**，只動 5 個檔（`scripts/t17r2_dataset_manifest.py`／`t17r2_blind_test.py`／`t17r2_rt60_table.py`／`t17r2_report_tables.py`／`test_t17r2_tools.py`）；`t17r2_common.py`／`t17r2_make_player.py`／`src`／`data`／既有 t17 四支零 diff；未跑任何真實照片、未建立 `output/mvp_acceptance_r2/`。
> - **R1**：刪 (a-4) 假斷言；真實突變（MANIFEST 抄 forced 的那行改寫死 `False`）→ 只有 (a-3) ❌、`EXIT=1`；`cp` 還原＋`cmp` 相同＋重跑 `EXIT=0`，輸出原文在 T-57-F1 卡交接筆記。**R2**：三處誤報更正另起新行追加（不刪原文）；四條 `md5 -q` 全中（`2adbaa75…`／`2dd19b6e…`／`9a94ffdf…`／`a1c21bcc…`）。
> - **R3～R6、第 7 條（裁定 T-57-D）、第 8 條**：測試共 74 條斷言全過；舊碼 `4d4f63b` 上 36 條 ❌（11 條「原行為不變」guard 仍 ✅，交接筆記逐條標明）。分母口徑：主率＝❌÷可判面數、6N 上下界同列、可判＝0／N＝0 不印 0%（可判＝0 時上下界只印分數，取捨見交接筆記「執行時的決定」1）。
> - **給 Opus 特別複核**：交接筆記「執行時的決定」1～8；加碼項（R4 的缺 `in_domain`／壞 JSON、`--dry` 指到目錄、`_repo_relative` 文字優先算法）是獨立複審抓到的同型缺口、非卡片條文；另做了一輪 8 個唯讀 agent 的複審與 51 個突變體（不取代 Opus 驗證）。
...
>
> - 四軸：工程：已驗證｜實驗：負向（H1 不支持｜H2 支持｜H3 不支持｜H4 不支持）｜產品：不適用｜MVP：不適用。審查 HEAD `1d5ca73`。
> - 上一輪退回的兩件紀錄問題已補正並經機械核對：`change_record` 行還原後 T-58 卡對 `81158cd` 零刪除行（C1／C2 空、C4 恰 1 行、C5 `exit=0`）；grep／`--stat`／`@@` 原文補貼且 C6×4＋C7 全 `exit=0`。
> - 程式與 REPORT 未再動：唯讀 sha256 全中、HEAD 複本重跑 `report` 逐位元相同、22 支測試 EXIT=0。Fable「不走 §7」判斷 Opus 同意。
> - 不影響 T-17-R2 排程（T-58 系列從未是 R2 前置）；`IR_RT60_BASIS`（維持 `"sabine"`）與保留號 T-59 照 Fable 處置留到 R2 之後。詳見 TASKS.md T-58-F2 卡末「✅ Opus 驗證紀錄」與 DEV_LOG `2026-09-18 (163)`。
...
>   做三件事：① T-58 卡 §8 `change_record` 行還原為 `81158cd` 原字、F1 接在行尾的那句**逐字搬到下一行**（卡上有三行逐字樣板）；② T-58-F1 交接筆記**另起新行**追加更正（「整份 diff 零刪除行」作廢）＋兩處補件指標；
>   ③ 補貼 Q2 兩份原文——grep 改對固定 commit `31233ed` 取快照（31 行＋逐行五類標註）、`--stat`／`@@` 行 7 條。每個貼上去的區塊都有機械比對式（C6／C7，必須 `exit=0`）。
> - **Fable 決定**：`change_record` 照 Opus 建議還原，**驗收條件不改、不走 §7**（兩條指示可同時滿足，門檻沒錯；改讀成「字元層級只追加」才是放寬門檻）——這個判斷交 F2 的 Opus 驗證獨立核對。
>   開卡瑕疵（F1 卡寫「其後追加一句」沒寫另起新行）記錄在案、不構成豁免。
> - **新共同鐵則（Phase 1.9-R 第 16、17 條，所有模型適用）**：**16**「只追加」＝只新增整行，不得在既有行行尾／行中加字，驗收用行層級機械檢查（狀態／四軸兩欄位例外）；
>   **17**「貼實際輸出」＝指令列＋stdout 逐字全文，結論句只能寫在輸出之後；會被自己貼上去的內容改變的指令，一律改對固定 commit 取快照。
> - **檢查式 Fable 已在 scratchpad worktree 模擬實跑**：修正前 C1 印 1 行／C2 印 2 行；模擬修正後 C1～C3 空、C4 恰 1 行、C5～C7 `exit=0`；反向測試（行尾接字）抓得到。Sonnet 步驟 1 的結果若與此不符＝卡關問 Fable。
> - **通過時** Opus 把 T-58／T-58-F1／T-58-F2 三卡四軸同時改「工程：已驗證」（實驗：負向不變），commit 訊息 `T-58: 驗證通過（工程）（含修正輪 T-58-F1／T-58-F2）`。再退回→Fable 開 T-58-F3。
> - **互鎖**：T-58-F2 不是 T-17-R2 前置，但 R2 步驟 2～3 之間不得有它的 commit（兩張卡都寫了）。本輪 Fable 改動全部是純新增行；零改動 `src`／`scripts`／`data`／`output`／SPEC／WORKFLOW。
>   詳見 TASKS.md T-58-F2 卡、T-58-F1 卡「🔮 Fable 處置」、共同鐵則 16／17 與 DEV_LOG `2026-09-18 (161)`。
>
> ## 🟠 2026-09-18 Opus：T-58（含修正輪 T-58-F1）工程仍退回——**現在該做的是：開 Fable 視窗開 T-58-F2**（T-57-F1 照常，兩者互不相依；都不是 T-17-R2 前置）
>
> - **程式與 REPORT 沒問題**（Opus 逐項獨立重跑）：判定程式、REPORT、tables 逐位元重現；無手打（竄改快取後文字全部跟著變）；R2 四組合、R3 恰 3/6 新舊碼對照成立；
>   唯讀快取／manifest／28 條 WAV sha256 全中；22 支測試 EXIT=0；`src`／`data` 零 diff。Fable「R3 不屬 §7 變更」Opus 同意。
> - **退回只因紀錄**：Q1 R1 驗收「T-58 卡零刪除行」未達（§8 `change_record: 無` 被就地改成 `無（T-58-F1：…）`），交接筆記卻寫「零刪除行」＝不實；Q2 自我檢查 5 的 `@@` 行與 R1 grep 全部輸出未貼原文。
> - 四軸：工程：退回｜實驗：負向｜產品：不適用｜MVP：不適用。建議 F2 純文件、不改腳本、不重跑 `report`。詳見 TASKS.md T-58-F1 狀態欄與 DEV_LOG `2026-09-18 (160)`。
>
...
>   R2 H3 MIT 子集「方向不一致」字樣錯；R3「多數」被寫成「≥半數」；R4 Part A pra 位置差異未註明；R5 REPORT §0／§3 沒交代結果。詳見 TASKS.md T-58 狀態欄。
> - 四軸：工程：退回｜實驗：負向｜產品：不適用｜MVP：不適用。**下方 Sonnet 區塊的「H4 部分支持」已被本次審查更正為「不支持」。**
>
...
>   主率＝❌÷**可判面數**、同列 N／6N／可判／無法判＋以 6N 為分母的上下界；「無法判」只限 GT 缺或 GT unknown；可判＝0／N＝0 不得印 0%。
>   核准者＝Fable（Opus 提出、使用者指派）；不需使用者另行核准，但**使用者可在 T-17-R2 步驟 1 前回一句「分母改固定 6」否決**。
> - **R5＝選項 (i)**：T-17-R2 步驟 2(e) 新增 log 格式——預設路徑那次存 `runs/<run>.log`、**末行 `exit=<整數>`**（指令樣板逐字照抄）；forced 重跑另存 `<run>.forced.log`。
> - **T-57-F1（Sonnet）**：R1～R5＋Fable 補列 R6（manifest `--dry` 沒接線）＋分母文字＝計算＋兩個小問題；每條有驗收方式；追加自檢「真突變兩段輸出」「新斷言在 `4d4f63b` worktree 上 fail」。
>   通過 Opus 驗證時 commit 訊息用 `T-57: 驗證通過（工程）（含修正輪 T-57-F1）`——**在那之前 T-17-R2 前置「T-57 ✅（工程）」不成立**。
> - **🗑️ 背景調查任務 `task_14c97967` 可以撤銷**：它的前提「T-20／T-21 四條 MD5 漂移」是誤報（拿 SHA-256 前 14 碼比 MD5；Opus 重生四條 MD5 全中）。
>   使用者在 App 的背景任務清單看到它時直接關掉／忽略即可；若它已經跑出任何結論或 commit，一律不採用、回報 Fable。「同碼同 seed＝逐位元相同」前提仍成立。
> - T-58 繼續平行（另一視窗進行中：`output/rt60_basis_probe/` 有未追蹤產物，**本輪未碰**）；與 T-57-F1 同時收工時各自只 `git add` 自己的檔案。
> - 本輪零改動 `src`／`scripts`／`data`／SPEC／WORKFLOW；只改 TASKS／DEV_LOG／HANDOFF／TODO／ROADMAP。
>
> ## 🟠 2026-09-18 Opus：T-57 工程退回——**現在該做的是：Fable 先裁定錯誤放行率分母口徑（§7），再開 Sonnet 跑 T-57 修正輪（R1～R5，見 T-57 卡「狀態」）**
>
> - ⚠️ **更正下方 Sonnet 段落**：「六條 IR MD5 有 4 條與歷史不同／疑似套件漂移」是**誤報**（拿 SHA-256 比 MD5）。Opus 重生四條 MD5 全中，
>   「同碼同 seed＝逐位元相同」前提仍成立；背景調查 `task_14c97967` 可撤銷。
> - T-17-R2 前置「T-57 ✅」**尚未成立**。
>
...
> - 等使用者：① T-04 補來源網址或明確「維持未結案」；② 提供 ≥5 張 held-out 五類照片或明確「沿用舊五張（降級）」；③ 核准被 gate 擋下照片在 §7-1／§7-2 的處理程序（Fable 建議 P1）。全文見 TASKS.md T-17-R2 卡「🔮 Fable 複評 T-17-R2-pre」§2。
> - Fable 複評：MINC/DMS 不開（R2 後再議）、陳設公式修正輪不插隊。**誠實預告：default 13/13 BLOCK，R2 期望結果是 MVP FAIL（R2）**，仍建議跑（held-out 基線先於任何調參）。
> - T-56 必須在 T-17-R2 樣本產生前結案（provenance 是 HEAD 精確比對）。
...
>   新測試舊碼 (a) fail；21 支測試全過；六條 IR MD5 全中。
> - 四軸：工程：已驗證｜實驗：正向（僅限 expected_on_13＋expected_V5，非校準）｜產品：預設啟用｜MVP：不適用。非阻擋更正 3 點見 T-54 卡「✅ Opus 驗證紀錄」第 6／8／9 點、DEV_LOG (145)。
> - ⚠️ 本輪 Opus 與執行者同一對話（中途切換模型），全部重跑以補足獨立性；需要的話可另開 Opus 新視窗再審。
> - 給 T-55 的提醒：照片 CLI 每跑一次都會自動在 `output/.archive/<stem>/` 建備份條目，清理清單要如實列出（不刪）；不要寫「未碰」。
...
> - 四軸：工程：已驗證｜實驗：**負向**（A：RacquetballCourt4 域外出口誤放 FAIL → T-54 修、T-55 重驗；B：v2-a 同義反覆、v2-b inconclusive → T-56）｜產品：不適用｜MVP：不適用。
> - T-48 §8 reviewer／verdict 已填；T-48-S (c) 重跑規則的基準 commit 建議取 `012a07f`（量測數字首次進版控）。
...
> - 待修：Q1 `output/material_r2/REPORT.md` §3 首段「官方判定只用唯一交付的那次」與 §0 矛盾（腳本 `:666`）；Q2 兩份 REPORT 檔頭需另列量測 commit
>   （A＝`714703d`、B＝`cda6b9b`）並註明 report-only 重產，B 內文「本次重生／本次執行／搬移前後」（`:760`／`:762`／`:801`／`:802`／`:805`）依模式改寫；
>   Q3 卡片「4 次額外重跑」改為至少兩輪（`dd03c0e`、`cda6b9b`）。N1～N3 順手。指示全文見 TASKS.md T-48 卡「🟠 Opus 修正輪複驗紀錄」。
> - 四軸：工程：退回｜實驗：負向（A FAIL；B v2-a 正向〔同義反覆〕、v2-b inconclusive）｜產品：不適用｜MVP：不適用。T-52 第三輪複核與 T-54～T-56 草案核准照舊平行。
...
> 與文件、Opus 列出的矛盾處」：R1／R2 直接更正卡片交接筆記文字（原文一字未刪）；R3／R4／R5／R7 是
> `scripts/t48_geometry_material_r2.py` 的報表產生邏輯修正（commit `8bfe262`，未改任何量測數值、未改判準、未碰
> `gen_ir_manual.py`／`geometry.py`）——新增 `partA-report-only`（讀既有 `output/geometry_r2/runs/` log 重產報表，
> 不重跑 CLI）、沿用既有 `partB-report-only`（讀既有交付 WAV，不重生 IR）；R6 以「→ Opus 更正（2026-09-14）：…」
> append 方式修正 T-11／T-12 §8 的錯誤 commit 引用（原文不刪）；R8 六條交付 IR MD5 本輪全部實跑驗證（`--text 浴室`／
> `--text 大教堂`／`--scene neighbor_voices.json`／`--scene stadium_corridor.json` 四條 MD5 全中，T-14 兩條由
> 20 支測試套件內建涵蓋），不再只驗 2 條。F3（裁決 T-48-F）：`car_interior_suv` 的 v2 判定改記 inconclusive（判準
> 文字自相矛盾），REPORT／T-11 §8／腳本標籤三處同步，`v2_category` 分類鍵未動、只改顯示文字。
>
> **紅線全部遵守**：`src/`／`data/` 全程零 diff；Part A 未重跑 CLI、Part B 未重生任何 IR；`output/geometry_r2/runs/`／
> `output/material_r2/*.wav` 全程未碰；未預先執行 T-54／T-55／T-56 任何步驟；20 支 `scripts/test_*.py` 全 `EXIT=0`。
> 清理：本輪未新增任何暫存路徑（R8 驗證覆寫的 4 個 `output/` 子目錄本輪開始前即存在、覆寫後 MD5 與覆寫前相同，
> 無需清理；`output/.archive/` 本輪未碰）；原始執行輪〔`012a07f` 那個視窗〕實際刪除的路徑清單本輪**無法回憶**，
> 如實標 ⚠️（Fable 修正輪指示第 5 條）。
>
> **下一步**：開 Opus 新視窗，依 T-48 卡「Sonnet 修正輪指示」1～5 條＋裁決 T-48-F 追加 6～8 條逐項複驗（對象＝本次
> 收工 commit）→ 通過後「工程：已驗證｜實驗：負向（A FAIL；B v2-a 正向〔同義反覆〕、v2-b inconclusive）」→
> T-54 → T-55 → T-17-R2（T-54／T-55／T-56 三份 criteria 草案仍等使用者核准，本輪未動）。詳見 TASKS.md T-48 卡
> 「交接筆記（修正輪，Sonnet 執行，2026-09-14）」與 DEV_LOG `2026-09-14 (136)`。
...
>   不是變更，不走 §7）。role_aware 模式 **0.4 不得視為已校準、且以現有資料無法校準**（唯一跨線 0.4044 判錯、放行案例 6 面錯 2、已知錯誤放行 1/5、
>   9 面膨脹、geometry 軸亦受影響、13 張同時是 T-44 調參集＝過擬合紅線）。產品軸：role_aware 維持 🧪 預設關閉，升級條件追加 T-53 校準 PASS。
> - **等使用者核准（Fable 不自改自批）**：**甲**＝不改碼，明文「role_aware gate 未校準、放行不可信」；**乙（Fable 建議）**＝gate criteria v2
>   「role_aware 收窄候選集的 clip 面不計入放行」＝T-47 ⑦(b) 正式化，執行卡 **T-52**（Sonnet），使用者核准後 Fable 先獨立 commit
>   `criteria: gate v2 …` 再開跑。丙（⑦a 縮放）／丁（gate 用全域集判）已否決，理由見裁決 2.2。
> - **附帶發現 ⓐ～ⓕ 全部處置**（裁決第 4 節）：ⓐ 採納（2/6 錯為準，併入 T-44-R1／T-17-R2 產表）；ⓑ 不改規則 1（維持 T-36-A；改了會動 default／T-46 基線）
>   記已知侷限＋報告項；ⓒ 採納為否決丙理由；ⓓ 採納為證據 ⑧；ⓔ 有效近門檻面 12 非 14；ⓕ 併入 T-53 規格。
> - **排程**：T-44-R1 ⏸ 移出關鍵路徑（等 T-53＋held-out）；T-17-R2 前置「裁決 T-47-A」自此滿足、以預設模式重驗；**T-48 可立即開跑**（與 T-52 檔案不相交）。
> - 本輪零改動：`src`／`scripts`／`data`／`output/gate_calibration/*`／WORKFLOW／SPEC；未提交任何 `criteria:` commit；未寫 MVP PASS。
>   詳見 TASKS.md T-47 卡「🔮 裁決 T-47-A」、T-52／T-53 卡、DEV_LOG `2026-09-14 (127)`。
...
> 重跑 tables.md 與提交版零差異、表 1～6 sha256 三版相同、52 列獨立重算一致、反向測試證明 gate 欄非寫死、
> 20 支測試 exit 0、六條 IR MD5 全中。**仍退回的唯一原因**是上一輪第 2 點「WORKFLOW §8 不可變欄位缺漏」
> 至今沒有 Fable 裁定，依 §5.4.1 與紅旗 7 不能以附註略過。
>
> **下一步**：開 Fable 視窗，請它對 T-47 的 §8 缺漏二選一並落卡——(i) 本卡追加「事後補建」§8 區塊；
> 或 (ii) 走 WORKFLOW §7（使用者核准、獨立 commit）修 §8 文字，讓無門檻量測卡不適用。之後 Opus 只複核
> 該區塊與範圍即可轉「已驗證」，量測不必重跑；再由 Fable 下裁決 T-47-A（附帶發現 ⓐ～ⓕ 一併參考）。
> 詳見 TASKS.md T-47 卡「🟠 Opus 第二輪複驗紀錄（2026-09-14）」與 DEV_LOG `2026-09-14 (122)`。
...
> 沒有 gate 欄，「(b) 會把 `bathroom_tiled` 收回 `BLOCK`」這個結論只出現在手寫交接筆記／
> DEV_LOG，不是程式產出；②WORKFLOW §8 不可變欄位缺漏，交 Fable 定奪。**本輪只修第 1 點**，
> 第 2 點與 Opus 附帶發現 ⓐ～ⓕ 原樣交 Fable，不擴大範圍。
>
> 只改 `scripts/t47_gate_calibration.py` 產表／產報告部分：表 7（⑦a）把原本誤稱「模擬
> gate」、實際是信心值（low/medium）的欄位改名「模擬 overall」，新增由程式唯讀呼叫
> `_overall_confidence()` 換算出的真正「模擬 gate」（BLOCK/pass）與「實際 gate」（取自
> CLI `analysis.json` 既有的 gate 結果）；表 8（⑦b）同樣新增這三欄，模擬前先用
> `_strip_conf_label()` 取出 `simulate_narrow_clip_downgrade()` 可能帶註解字串裡的純
> low/medium/high（避免 `_overall_confidence()` KeyError）。REPORT ⑦(a)(b) 各加一句程式
> 填入的「模擬後 pass 張數／實際 pass 張數」。模擬邏輯本體（`effective_threshold`／
> `simulate_method`／`build_surface_from_sim`／`simulate_narrow_clip_downgrade` 的判定規則）、
> 表 1～6、`src/`、`data/` 一律零改動。
>
> **重跑前先 commit 腳本**（`5c1cd29`）讓工作區乾淨，再 `--fresh` 重跑（52 次真實模型推論，
> 約 17 分鐘）exit 0，26/26 交叉核對相符、T-44 第四輪 9 面信心上升交叉檢查 9/9 通過。
> **程式化比對表 1～6**（`## 表 7` 之前的內容）與退回前提交版 `6d95f5f` **sha256 逐位元
> 相同**（`b7baec61…`）。新表 7／8 的程式化結果與退回前手寫結論一致：⑦(a) 兩模式模擬後
> 13 張皆 0 pass；⑦(b) `role_aware` 模式 `bathroom_tiled` 由實際 `pass` 模擬變 `BLOCK`；
> REPORT 程式填入 `default` 0/0、`role_aware` 0/1（兩個模擬都把唯一一張 pass 收回）。
>
> **自我檢查**：20 支 `scripts/test_*.py` 全 `EXIT=0`；六條交付 IR MD5 全中（T-14 由
> `test_ir_synth.py`【6】內建；T-20／T-21 四條分開各一行重生＝`2adbaa75…`／`2dd19b6e…`／
> `9a94ffdf…`／`a1c21bcc…`，逐位元與歷史記錄相同）；`git diff --stat 795e348 HEAD -- src data`
> 為空（本輪起點 `795e348`）；`git status --porcelain -- src scripts data` 於 `--fresh`
...
> ## 🔵 2026-09-11 Sonnet：T-47 gate 校準複審量測完成——**現在該做的是開 Opus 新視窗複驗**，結果 commit `6d95f5f`
>
...
> 填 `6d95f5f`**。通過後 Fable 才能依四樣證據下裁決 T-47-A，之後才能開 T-44-R1。
> 詳見 TASKS.md T-47 卡「交接筆記（Sonnet 執行，2026-09-11）」與 DEV_LOG
> `2026-09-11 (120)`。
...
>
> **下一步**：開 Opus 新視窗，貼 WORKFLOW §2.2 v2 複驗 Prompt，**「結果 commit」填
> `bad3f98`**。通過後 T-47 前置（「T-42／T-43 ✅」）才算滿足。詳見 TASKS.md T-43 卡
