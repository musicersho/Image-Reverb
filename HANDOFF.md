...
> 最後更新：2026-08-27（Fable 裁決視窗）｜對應 commit：見 `git log --oneline -1`
> **新視窗請先讀 [CLAUDE.md](CLAUDE.md) 知道自己的角色，再讀本檔知道現在的狀況。**
>
> 驗證本檔是否過期：看 [DEV_LOG.md](DEV_LOG.md) 最上面一筆是不是 `2026-08-27 (18)`；
...
**T-10 ✅｜T-12 ✅｜T-11 🔵 決策補丁完成＋Fable 裁決結案，待 Opus 驗證｜
T-13 🔵 已完成（並行視窗 2026-08-26），也在待 Opus 驗證。**

🔮 **T-11 決策補丁已執行完（Sonnet 2026-08-26）且 Steinman 卡關已裁決（Fable 2026-08-27）**：
量程規則已實作（`GEOMETRY_SCOPE_MAX_M = 10.0`，透視任一維／環景單面牆距超標 → low ＋警示），
複測 A'（浴室 +24%）、B'（走廊/車內/體育館全部 low，走廊靠規則非 hardcode）皆通過。
Steinman 翻 low 曾卡關——裁決：**那是規則該有的行為**（實測三面牆距 12.2/10.4/11.1m 超標，
原決策文字「未超標」是總長÷2 的粗估錯誤）；規則不改，對照組驗收語句修正為
「浴室＋三個範圍內透視場地維持 medium」（複測全過）。**環景解「視野外」、不解「量程」。**
裁決全文在 TASKS.md T-11 卡「Fable 裁決」；SPEC 升 v0.3.1（§7-2 措辭涵蓋環景超標場地）。
...
## 0. 【現在該做的事】開 Opus 視窗驗證 T-11 與 T-13（建議分開兩個視窗、各自獨立審）

> **模型選 Opus**，把下面這段貼進去：

```
你是驗證者，只審查不修改程式碼。請依 WORKFLOW.md 第 5 節的驗證標準，
審查任務 T-11 的成果：讀 TASKS.md 的該任務卡（含「Fable 路線決策」「Sonnet 決策補丁
執行結果」「Fable 裁決」三節）、讀 output/geometry/REPORT.md §8、讀相關程式碼、
實際執行驗證指令。最後輸出：「✅ 通過」或「❌ 退回」＋具體理由。
```

> **T-11 的 Opus 驗證重點**（裁決後版本，與 TASKS.md 卡上一致）：
> 1. 走廊翻 low 是靠量程規則觸發（估值 12.8/14.8/14.7m > 10m），不是 hardcode 場地名（紅旗）。
> 2. 防濫殺對照＝浴室＋樓梯間＋AI 臥室＋洞穴實驗室維持 medium；
>    Steinman 依裁決預期為 **low**，觸發值必須是 JSON 裡的實際牆距（12.2/10.4/11.1m）。
> 3. 三個門檻未被動過：`GEOMETRY_ERROR_TOLERANCE` 0.30、`CLIP_CONFIDENCE_THRESHOLD` 0.4、
>    `GEOMETRY_SCOPE_MAX_M` 10.0（與決策文字一致）。
> 4. REPORT §8 的數字可重跑重現（指令在 REPORT §7）。

> **T-13 驗證另開一個 Opus 視窗**，貼同樣的標準驗證 Prompt、把任務編號換成 T-13。
> T-13 的紅旗在卡上：程式裡不得存在 mean(α)→RT60 路徑（地雷 #8）、
> Sabine 與實測 IR 低頻差 2 倍的處理（地雷 #14，卡上步驟 4b）。
...
| T-11 | 幾何估計（metric depth → 房間尺寸） | 🔵 **待 Opus 驗證**（補丁完成 2026-08-26；Steinman 卡關經 Fable 裁決結案 2026-08-27） |
| T-12 | 材質模組（逐表面材質） | ✅ **通過**（Opus 驗證 2026-08-25，含使用者試聽 ✅） |
| T-13 | 聲學參數計算（逐頻段 RT60 + pre-delay） | 🔵 **待 Opus 驗證**（並行視窗完成 2026-08-26，自檢全過） |
| T-14~T-17 | Phase 1 其餘任務（已細化） | ⬜ 等 T-11/T-13 過驗證後依序開工 |
...
【現在該做的 — 驗證 T-11】（模型選 Opus）
  貼本檔第 0 節那段 Prompt（已含裁決後的驗證重點）

【也該做的 — 驗證 T-13】（模型選 Opus，另開視窗）
  貼：「你是驗證者，只審查不修改程式碼。請依 WORKFLOW.md 第 5 節的驗證標準，
      審查任務 T-13 的成果：讀 TASKS.md 的該任務卡、讀相關程式碼、實際執行驗證指令。
      最後輸出：「✅ 通過」或「❌ 退回」＋具體理由。」

【兩卡都過驗證後】回 Fable 視窗確認 T-14（IR 合成引擎）開工
```

### ⚠️ 給下一個 Opus 視窗的提醒

- **T-11 的驗收語句經 Fable 裁決修正過**：Steinman 預期是 **low** 不是 medium
  （TASKS.md 卡上有裁決全文與修正對照）。不要拿舊語句（含本檔歷史版本）當判準。
- T-13 的紅旗：程式裡不得存在 mean(α)→RT60 路徑（地雷 #8）；
  Sabine 與實測 IR 低頻差 2 倍的處理是否照卡上步驟 4b（地雷 #14）。
