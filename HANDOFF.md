...
（全部經 Opus 驗證）｜T-16 🔵 待驗證（Sonnet 自檢通過，2026-08-30）｜
T-18 / T-17 ⬜ 未開始（依此順序；T-18 可提前插入，T-17 前必過）。**

🎯 **現在該做的：Opus 驗證 T-16**（WORKFLOW §2.2 標準 Prompt）。T-16 新增
`src/image_reverb/visualize.py`，三種輸入各自的拼版 `analysis.png`；PNG 上的數字
全部直接讀 `analysis.json`（不重算），唯一重跑模型的地方是分割疊色圖／深度圖的
像素資料（做法與既有 `scene_cues` 重跑同模式）。細節與自我檢查結果見 TASKS.md
T-16 卡「交接筆記」。**通過後依序 T-18（可提前插）→ T-17。**
...
| T-16 | 分析視覺化（三種輸入各有拼版） | 🔵 **待驗證**（Sonnet 自檢通過，2026-08-30） |
...
【現在該做的 — 開 Opus 視窗驗證 T-16】（模型選 Opus）
  貼 WORKFLOW §2.2 驗證 Prompt，驗證目標是 T-16。

【T-16 通過後】依序：T-18 → T-17（T-18 可提前插入，但 T-17 前必須通過驗證）。
```

### ⚠️ 給下一個視窗的提醒

- **T-16 已完成 Sonnet 自檢，待 Opus 驗證**：新增 `visualize.py`，三種輸入各自的
  `analysis.png`；PNG 數字全部直接讀 `analysis.json`，唯一重跑模型的地方是分割
  疊色圖／深度圖像素資料。驗證重點與已知的非阻斷觀察（`coupled.py` 既有 3/4 位
  小數落差、`output/<name>/` 同名覆寫不清舊檔）見 TASKS.md T-16 卡交接筆記。
