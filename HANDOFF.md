...
> 最後更新：2026-08-27（Opus 驗證 T-11 視窗）｜對應 commit：見 `git log --oneline -1`
> **新視窗請先讀 [CLAUDE.md](CLAUDE.md) 知道自己的角色，再讀本檔知道現在的狀況。**
>
> 驗證本檔是否過期：看 [DEV_LOG.md](DEV_LOG.md) 最上面一筆是不是 `2026-08-27 (21)`；
...
**T-10 ✅｜T-11 ✅｜T-12 ✅｜T-13 ✅ —— Phase 1 前四卡全部通過 Opus 驗證。**

🎯 **現在該做的：回 Fable 視窗確認 T-14（IR 合成引擎 v1）開工。沒有待驗證的卡了。**
...
並在 DEV_LOG.md 記錄規劃決策。目前的情況是：Phase 1 的 T-10/T-11/T-12/T-13 全部通過
Opus 驗證，尺寸來源、逐表面材質、逐頻段 RT60 三個上游都已定案，請確認 T-14（IR 合成
引擎 v1）的任務卡是否需要依這三卡的實際產出調整，然後我要開 Sonnet 視窗執行 T-14。
```

> **給 Fable 的重點提醒**（來自已完成四卡的實證）：
> 1. **地雷 #14**：Sabine 公式與實測 IR 在低頻差 2 倍以上。T-13 已用
>    `rt60_source: "formula"` ＋ disclaimer 標示，**T-14 合成 IR 時要正面處理**。
> 2. **SPEC §7 第 4 條**：每次改動 IR 生成邏輯後必須產生試聽檔請使用者實聽。
>    T-14 直接生成 IR，這條一定會用到（數字合理 ≠ 聽起來對，已吃過一次虧）。
> 3. T-13 驗證發現「零/負尺寸會安靜回傳 rt60 全 0 但 confidence 仍 high」，建議 T-15 補防呆。
> 4. T-11 驗證發現量程規則是「預設放行」（不認得的 `dims_source` 會靜默跳過檢查），
>    建議改成預設攔截（記在 T-11 卡建議 2）。
...
| T-14~T-17 | Phase 1 其餘任務（已細化） | ⬜ **T-14 可開工**（上游三卡已全部定案） |
...
【現在該做的 — 確認 T-14 開工】（模型選 Fable）
  貼本檔第 0 節那段 Prompt

【Fable 確認後 — 執行 T-14】（模型選 Sonnet）
  貼：「執行 TASKS.md 的任務 T-14。先讀 CLAUDE.md 和該任務卡的全部內容再動工，
      完成後執行任務卡裡的自我檢查，最後照 WORKFLOW.md 第 4 節做收工程序。」

【T-14 做完後】（模型選 Opus，另開視窗）
