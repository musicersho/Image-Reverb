...
> 最後更新：2026-08-30（Fable 視窗：零信用複驗＋裁決 T-28-A＋本檔全面更新）
> **新視窗請先讀 [CLAUDE.md](CLAUDE.md) 知道自己的角色，再讀本檔知道現在的狀況。**
>
> 驗證本檔是否過期：看 [DEV_LOG.md](DEV_LOG.md) 最上面一筆是不是 `2026-08-30 (55)`；
...
🎯 **現在該做的：Sonnet 執行 T-30（gate 出口導引）**——只改 gate 擋下後印什麼：
逐面點名 `fallback`／`out_of_domain` 面、依軸給建議、附可直接複製的
`--override-material` 覆寫指令骨架（實測這是唯一可行的非 force 出口，現有訊息
卻只提單獨走不通的 `--override-dims`）。**紅線：gate 判定規則一行都不許動、
六條交付 IR MD5 不變。** 卡片在 TASKS.md 最尾。
做完 → Opus 驗證 T-30 → 回 Fable 裁決 T-27（室內陳設吸音，材質修正輪的前置）。

⚠️ **排隊中的另外兩件**：T-27（🔮 等 Fable 裁決：等效吸音面積 vs occupancy，
臥室被聽成教堂的結構性根因）、T-29（三條管線 `analysis.json` schema 不一致：
三軸信心只在照片管線，`--text` 只有 `confidence`、`--scene` 連 `confidence`
都沒有；未裁決）。
...
無來源 11；修好材質規則仍有 6/13 被 geometry=low 擋）。
...
| T-30 | gate 出口導引（裁決 T-28-A 執行卡） | ⬜ **未開始 ← 現在做這張**（Sonnet） |
| T-27 | 室內陳設吸音表示 | 🔮 等 Fable 裁決（材質修正輪前置） |
...
【現在該做的 — 開 Sonnet 視窗執行 T-30】（模型選 Sonnet）
  貼 WORKFLOW §2.1 標準 Prompt，任務編號 T-30。

【T-30 完成後 — 開 Opus 視窗驗證】（模型選 Opus）
  貼 WORKFLOW §2.2 驗證 Prompt，驗證目標 T-30。

【T-30 通過後 — 回 Fable 裁決 T-27】（模型選 Fable）
  T-27（室內陳設吸音：等效吸音面積 vs occupancy）是材質修正輪的設計前置；
  材質輪做完後依裁決 T-28-A 用新基準率複測 13 張，再評估 gate 規則。
```

### ⚠️ 給下一個視窗的提醒

- **T-30 的紅線**：gate 判定規則（`compute_materials_confidence()` 與
  `run_photo()` 的觸發／放行條件）**一行都不許動**，本卡只改「擋下之後印什麼」。
  Opus 驗證用六條交付 IR MD5＋`test_output_gate.py` 既有【A】【B】【C】案例抓偷改。
- **動信心規則前必讀地雷 #23／#24**：材質來源有第四種「無來源」狀態
  （不觸發規則 1、永久阻斷規則 3，**不要**列進 T-30 的覆寫建議）；
  透視照的 materials `high` 結構性不可達（規則 3 條件恆假）。
- **重生照片 IR 要知道 gate**（REPORT §7 已註記）：2026-08-30 後重跑要加
  `--force-low-confidence`，或依第 5 節用 `--override-material` 走正規出口。
- **任何再調 gate 門檻的提案**：依裁決 T-28-A，必須等材質輪後用新基準率
  複測 13 張並附實測，否則就是 WORKFLOW §5 紅旗 3；臥室那筆必須仍被擋住。
