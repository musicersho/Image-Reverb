...
- **狀態**：🔴 **卡關（Sonnet，2026-08-26）**——決策補丁步驟 7–8 已完成且核心判準 A'/B' 皆通過，
  但步驟 8 明文要求的「Steinman 防濫殺對照組維持 medium」未通過（補丁後翻成 low），
  如實記錄、未調整規則讓它通過，需 Fable 確認如何處置（詳見下方「Sonnet 決策補丁執行結果」
  與 [`output/geometry/REPORT.md`](output/geometry/REPORT.md) §8.4）。
  歷程：🔴 卡關（2026-08-18，評測關卡判準 A 未通過：走廊 −57%，根因是模型量程 ~20m 天花板）
  → Fable 依 REPORT §6 的 5 個方向定案（見下方「Fable 路線決策」）→ 本輪執行決策補丁。
- **前置**：T-10
...
- **🔮 Fable 路線決策（2026-08-25）**：**b 為主幹＋c 為正式出口＋d 保留；a 延後；e 拒絕。**
  ...
  - **補丁的 Opus 驗證重點**：走廊翻成 low 是靠量程規則觸發，不是 hardcode 場地名（紅旗）；
    浴室與 Steinman 沒被濫殺；`GEOMETRY_ERROR_TOLERANCE`、`CLIP_CONFIDENCE_THRESHOLD` 未被改動；
