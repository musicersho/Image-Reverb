...
- **狀態**：🔴 **卡關（評測關卡未通過，2026-08-18）** — 程式碼完成且可用，但步驟 5 的
  評測關卡判準 A（一般室內誤差 ≤±30%）**未通過**：走廊 −57%。
  **這是 T-08 決策一設計內的結果，不是實作失敗**（「metric 模型精度未驗證 → 先實測，
  不達標就 🔴 卡關回報 Fable」）。完整誤差表與決策材料在
  [`output/geometry/REPORT.md`](output/geometry/REPORT.md)。**請 Fable 決策後再往下走。**
- **前置**：T-10
...
- **狀態**：🔵 **待驗證（Sonnet，2026-08-18）** — 程式與自檢完成，
  **唯一未完成項是步驟 6b 的使用者試聽**（AI 不能代勞，試聽檔已產生並送出）。
- **前置**：T-10（可與 T-11 並行）
