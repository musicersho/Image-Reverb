...
- **⚠️ 後續發現的適用尺度上限（2026-08-28，Opus 驗 T-21 時發現，本卡狀態不變）**：
  早期窗 `IR_EARLY_MS=90` 與能量匹配窗（交接前 30ms）是固定值，當房間大到
  「最短一階反射比 90ms 還晚到」時，匹配窗是空的，晚期殘響會被縮放到近乎噪聲位準。
  同材質尺度掃描：40×30×15 誤差 −0%、80×60×25 −3%、120×100×35 **−75%**、
  160×130×45 **−94%**（2k/4k）。**引擎的適用尺度上限約 80–100m**，本卡驗證過的
  4×3×2.5 與 30×20×12 都在安全區內，且 `export_ir()` 的閉環報告會如實發出 >20% 警示
  ——但呼叫端若不跑閉環報告（T-21 的 `export_coupled()` 就沒跑）就會靜默。
  詳見 T-21 卡的退回理由 2。
...
- **🔧 修正清單（做完改回 🔵 待驗證）**：
  ...
  2. 決定巨蛋場景怎麼辦，二選一並記錄理由：(a) 把 T-14 的早期窗/能量匹配窗改成
     隨房間尺寸調整（`IR_EARLY_MS` 至少要涵蓋最短一階反射到達時間）——這會動到 T-14
     引擎，屬新任務；或 (b) 把示範場景改到引擎已驗證的尺度（≤80m 級），
     並在場景 JSON 註明為何不用真實巨蛋尺寸。
