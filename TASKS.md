...
- **狀態**：🔵 待使用者回饋（Opus 已執行，2026-08-30）。**§7-2 已完成且明確未達標**；
  §7-1／§7-3／§7-4 素材已備妥但**需要使用者本人操作**，尚無結果。
  依 **裁決 E**，照片來源網址未補齊前狀態最高停在 🔵，不得改 ✅。
...
- **⏳ 等使用者做的三件事**（做完回報給下一個視窗補進 REPORT）：
  ...
  3. **§7-4 試聽**：聽 `output/mvp_acceptance/listening/` 九個檔，
     **特別比較 `racquetball_court_4__wet.wav`（材質判錯）vs
     `DIAG_racquetball_correct_dims_and_hard_materials__wet.wav`（材質改對）**
     ——若第二個明顯更像壁球場，就用耳朵獨立確認了 §2.4 的病因診斷。
     請一併記錄有沒有地雷 #9 那種「拍鐵筒子」artifact。
  4. **📷 補齊 9 張照片的來源網址**（`assets/SOURCES.md` §2）——裁決 E 的結案前置。
- **🔮 給 Fable 的決策輸入**（REPORT §6）：
  ...
  待裁決：§7-2 未達標要加修正輪還是把目標移到材質模組升級之後？戶外空間要不要加拒絕出口？
  診斷 run 剩下的 500Hz–2kHz +28%～+74% 偏長要不要單獨開卡？
