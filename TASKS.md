...
- **狀態**：🟡 **路線已定案（Fable，2026-08-25），待 Sonnet 執行「決策補丁」步驟 7–9 後重過評測關卡**
  — 原實作保留不重寫。歷程：🔴 卡關（2026-08-18，評測關卡判準 A 未通過：走廊 −57%，
  根因是模型量程 ~20m 天花板）→ Fable 依 [`output/geometry/REPORT.md`](output/geometry/REPORT.md) §6
  的 5 個方向定案（見下方「Fable 路線決策」）。
- **前置**：T-10
