...
> **2026-09-10 Opus 更新（本檔最新一筆，優先於下面所有筆）**：**T-46 第四輪複驗通過——工程軸
> ✅ 已驗證（依 criteria v3 §5，對象＝結果 commit `7b1384e`）**。§2.7 分層判定＝**情形 ①**
> （只填一列）：Opus 自行 `--fresh` 39 次真實 CLI 後，`BASELINE.stable.md`／`tables.md` 的
> `git diff` **完全為空**，`BASELINE.md`／`REPORT.md` 的 diff 逐行確認全部落在 §2.2.4 provenance 項；
> §5.3 獨立重算 13/13 三者一致（兩輪 B0 逐葉比對唯一差異都是 `.elapsed_s`）；§5.4 另寫程式重驗
> A1～A7／B1～B2 全 13/13；19 支測試 EXIT=0、六條 IR MD5 全中、`bathroom_tiled` 不加 force EXIT=3；
> §3.3 紅線與新增的第八項紅旗全部不成立。**T-42／T-43／T-47／T-48／T-44-R1 的「前置 T-46 ✅」
...
> 2026-09-16 更新：B3 使用者明確決定不補（內部使用）→ 裁決 E 第二路徑，前置滿足；B2 改為 T-17-R2 §7-1 專用（GT 放 `assets/photos_heldout/ground_truth_heldout.json`，不進 `data/`），拍攝規格見 TASKS.md T-17-R2 卡「🔮 Fable 落地」§1；T-44-R1 仍等 T-53。
> 2026-09-20 更新：B3——使用者再決定「照片重新提供」→ T-04 卡裁定 T-04-R（v2：替換素材；Codex 已換圖；舊 9 張退役、來源缺永久列示）；R2 的 `T-04 缺項` 標示照舊。
> B2——使用者指定用五張 GPT Image 合成圖（路徑 S；長邊下限改 1280px；與開發素材共用、曝光已揭露；T-17-R2 卡裁定 T-17-R2-S），由 T-60 就位；T-44-R1 的 held-out 須另備。
