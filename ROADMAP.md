...
## Phase 1.5 — 場景描述輸入與複合場景（SPEC F-16 ~ F-17，2026-08-27 使用者需求新增）

目標：不用照片也能產 IR（文字描述），並支援「聲源與聽者在不同空間」的複合場景。
兩者都掛在既有中間表示之上，照片管線與 IR 引擎不改。任務卡 T-20/T-21 見 [TASKS.md](TASKS.md)。

- [ ] T-20 文字場景描述 → IR（scene preset 庫 + 關鍵字/參數解析；`dims_source: "text_description"`；
      認不得就報錯列清單，不安靜猜；不接外部 LLM API，本機 LLM 留待 Phase 3 評估）
- [ ] T-21 複合場景引擎 v1（路徑串接：聲源空間 IR ⊗ 牆體/開口傳輸濾波 ⊗ 聽者空間 IR，
      多路徑加總；`data/transmission.json` 傳輸損失表；示範場景＝巨蛋→走廊、隔壁人聲三路徑）
- 依賴：T-14（IR 合成引擎）。**排在 T-15 之前**，讓 CLI 整合一次收進三種輸入
  （照片／文字／複合場景 JSON）。⚠️ T-14 目前 🔵 待 Opus 驗證——若被退回，T-20/T-21 需複測。
