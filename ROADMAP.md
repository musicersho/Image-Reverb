# Roadmap

規格詳見 [SPEC.md](SPEC.md)，相關研究調查見 [RESEARCH.md](RESEARCH.md)。

## Phase 0 — 研究與可行性驗證 ✅（2026-08-16 完成，T-08 結案）

目標：確認「照片 → 可聽的 IR」整條路走得通，選定 MVP 的 IR 生成路線。

- [ ] ~~跑通 Image2Reverb baseline~~ ⏸️ 暫緩（T-07 選做，使用者未授權下載；不擋 Phase 1）
- [x] 測試單目深度估計 → **🔴 否定性結論**：相對深度不能估房間體積（`output/depth/REPORT.md` §7）
- [x] 測試語意分割 → **🔴 否定性結論**：ADE20K 的 `floor`/`wall` 材質語意不可信、車內無對應類別（`output/seg/REPORT.md`）
- [x] 建立材質 → 各頻段吸音係數對照表（12 種材質，`data/materials.json`）
- [x] 用 pyroomacoustics 手動參數生成 IR + 卷積試聽 → **使用者人耳確認整條鏈路可用**（T-02）
- [x] 建立「同場地照片 vs 真實 IR」評測集 → OpenAIR 已停站，改用 EchoThief + MIT Reverb Survey（8 場地）
- [ ] 細讀 AV-RIR 與 Differentiable Room Acoustic Rendering（2025）→ 移到 Phase 1 期間補
- [x] **決策點（T-08，2026-08-16 定案）**：
  1. IR 生成維持 **A+B 混合**（image-source 早期 + shaped-noise 晚期）——Phase 0 試聽已驗證此路線可聽
  2. 深度改 **metric depth 模型**（Depth-Anything-V2-Metric-Indoor），參考物尺度降為校驗用，手動覆寫升 P0
  3. 材質改 **兩階段**：分割管幾何角色、CLIP zero-shot 二階分類器管材質標籤，逐表面指定
  4. **環景支援納入 Phase 1 前處理**（equirect→多視角透視投影），解鎖 8 個驗收場地並提前處理視野外風險

## Phase 1 — MVP：照片 → IR 匯出（SPEC F-01 ~ F-09）

目標：CLI 工具，輸入照片、輸出可用的 IR (WAV)。任務卡 T-10~T-17 見 [TASKS.md](TASKS.md)。

- [x] T-10 專案骨架 + 影像前處理（letterbox 裁切、環景偵測與投影、HEIC 支援）✅ 通過（Opus 複驗 2026-08-18）
- [x] T-11 幾何估計模組（metric depth → 房間尺寸/體積，內建已知場地評測關卡、手動覆寫）✅ 通過（Opus 驗證 2026-08-27）
      — 🔮 **評測關卡卡關後 Fable 定案（2026-08-25）**：模型量程實證 ~20m 天花板，
      自動幾何適用範圍明訂 ≤10m；範圍外強制 low confidence ＋警示，出口為手動尺寸（F-09）
      或環景輸入；換 Metric-Indoor-Large 延後至 T-17 驗收後再評估（若屆時產品體驗不可接受）。
      補丁已執行（2026-08-26）；Steinman 對照組卡關經 Fable 裁決結案（2026-08-27）：
      **環景解「視野外」、不解「量程」**，單面牆距超標的環景場地正確地標 low。
      詳見 TASKS.md T-11 卡「Fable 路線決策」「Fable 裁決」、SPEC v0.3.1
- [x] T-12 材質模組（表面分割 + 二階材質分類 → **逐表面**吸音係數；「鐵筒子」缺陷已修並經人耳確認）✅ 通過（Opus 驗證 2026-08-25）
- [x] T-13 聲學參數計算（Sabine/Eyring → **逐頻段** RT60、pre-delay）✅ 通過（Opus 驗證 2026-08-27）
- [x] T-14 IR 合成引擎 v1（image-source 早期反射 + 分頻段 shaped-noise 晚期殘響）✅ 通過（Opus 驗證 2026-08-28；尺度上限由 T-22 解除）
- [ ] T-15 CLI 整合（**照片／文字／複合場景三種輸入互斥** → IR WAV + 分析報告 JSON，
      含手動覆寫、warnings/notes 分流、技術債收斂、交付 IR MD5 零回歸判準）
      — 🔮 卡片已於 2026-08-30 由 Fable 依 Phase 1.5 後實況改版
- [ ] T-16 分析視覺化（材質疊圖、深度圖、RT60 頻段圖、警示標示；文字/複合場景另有拼版）
- [ ] T-18 驗收前置（低頻聯合帶量測工具＋退出碼技術債；不依賴 T-15/T-16，T-17 前必過）
- [ ] T-17 **驗收**：SPEC §7 四項標準（含人耳試聽環節，Opus 主導）
      — 🔮 **§7-2 低頻判準已事前裁決（2026-08-30，Fable）**：500Hz–4kHz 逐頻段 <20% 不變；
      125/250Hz 門檻改為 88–354Hz 聯合帶 T30 <20%（逐頻段數字照列、超差照警示，只是不當門檻）。
      理由與證據鏈見 TASKS.md T-17 卡裁決 B。達標率依 dims_source 分組統計（裁決 C）；
      照片來源網址為結案前置（裁決 E）
      — ⛔ **首驗結果：MVP FAIL（2026-08-30；§7-1 2/5、§7-2 自動組 0/8／手動組 0/5），永久保留不覆寫；
      重驗＝T-17-R2（裁決 T-45-A，2026-09-03；硬前置 T-42／T-43／T-46／T-47／T-48／T-44-R1 與 held-out 素材）**
      — 🔮 **裁決 T-47-A（2026-09-14，Fable）**：default gate 維持不動；role_aware 0.4 未校準（使用者已核准乙，criteria `81bc4cd` → T-52）；
      T-44-R1 移出關鍵路徑（等 T-53 校準卡）→ T-17-R2 以預設模式重驗，前置改為 T-42／T-43／T-46／裁決 T-47-A／T-48／
      （T-52 結案或選甲）與 held-out 素材
      — 🔮 **裁決 T-48-F（2026-09-14，Fable）**：T-48 A 部分 RacquetballCourt4 域外誤放＝預設路徑安全缺口 → 開 **T-54**（量程規則 v2：環景分支加
      三維檢查）→ **T-55**（T-11 域外 v3 重驗：14 張＋V5 情境）→ 兩者列為 T-17-R2 前置；**T-56**（T-12 v2-b 量測方法 v3：seed 鎖定＋10 次中位數）
      不進關鍵路徑。三份 criteria 草案等使用者核准後才 commit。關鍵路徑：T-48 修正輪 → T-54 → T-55 → T-17-R2（T-52 Opus 複核 4(ii) 平行）
      — ✅ **2026-09-15 使用者核准三份 criteria**（`02284d9`／`b80a4fb`／`4a0b23e`；T-56 守門條款刪）；T-48 工程已驗證（`2ea4d41`）→ **T-54 可開跑**
      — 🔮 **2026-09-16 Fable 落地 T-17-R2**：T-54／T-55／T-56 全部結案；使用者回覆三項（T-04 內部使用不補網址／held-out 建議自拍／P1 依建議落地、貼 Prompt 即核准）；
      in-domain 事前定義（8 場地僅 `mit_gym`）；執行步驟 0～11；§8 前三欄已填。**T-17-R2 等 held-out 照片就位即開跑（Opus 主導）**，期望 `MVP：FAIL（R2）`
      — 🔮 **2026-09-18**：等照片期間開 **T-57**（R2 工具前置，Sonnet→Opus；T-17-R2 前置追加）與 **T-58**（調查卡：Sabine／Eyring／pra 參考／產品路徑四基準對照，只量不改，填充）
      — 🔮 **2026-09-18（同日）**：T-57 首驗 🟠 工程退回 → Fable 裁定報告項 5 分母口徑（裁定 T-57-D，`criteria:` `3007646`，結果前）＋開修正輪 **T-57-F1**（Sonnet→Opus）。關鍵路徑：T-57-F1 → Opus 驗 → T-17-R2（仍等 held-out 照片）；T-58 平行
      — 🔮 **2026-09-18（同日）**：T-58 首驗 🟠 工程退回（量測可重現，判定文字 R1～R5）→ 開修正輪 **T-58-F1**（Sonnet→Opus；只重跑 `report`；不進關鍵路徑）。
      實驗：負向——現有證據不支持把 `IR_RT60_BASIS` 由 sabine 換成 eyring／pra；**產品決定留到 T-17-R2 之後**。`ir_synth` 鄰帶耦合診斷登記保留號 **T-59**（R2 後開卡）
      — 🔮 **2026-09-18（同日）**：T-58-F1 複驗 🟠 二度退回（程式與 REPORT 已逐位元確認，只剩紀錄問題）→ 開純文件第二修正輪 **T-58-F2**（Sonnet→Opus；不改腳本、不重跑 `report`；不進關鍵路徑）；
      共同鐵則新增 16（只追加＝只新增整行）／17（貼實際輸出＝指令＋逐字輸出）。關鍵路徑不變：T-57-F1 → Opus 驗 → T-17-R2（仍等 held-out 照片）
      — 🔮 **2026-09-20**：T-57（含 T-57-F1）✅ Opus 驗證通過（`875697e`）。使用者下達兩項標準變更 → `criteria:` `758eeba`（裁定 T-04-R：T-04 改走「替換素材」v2，Codex 已換圖 `ba1fcdb`／`f3e07b0`，舊 9 張退役；
      裁定 T-17-R2-S：held-out 長邊下限 1920→1280px＋路徑 S＝AI 合成圖・與開發素材共用・曝光已揭露，判準 1～6 未動、Prompt B 不可用、共用圖禁用令）。新卡 **T-62**（換圖後測試相容修正）、**T-60**（held-out 素材就位），皆 Sonnet→Opus、關鍵路徑；T-61 取消。
      關鍵路徑：T-62 → T-60 → T-17-R2
      — ⛔ **2026-09-21 T-17-R2 結果：`MVP：FAIL（R2）`**（步驟 11 獨立複驗通過 `121697a`）：§7-1 盲聽 5/5（held-out＝AI 合成圖・共用開發素材）、§7-2 未達（自動組 0/0、coverage 0/1）、§7-3／§7-4 達成；
      放行的兩張 held-out 材質可判面錯 8/10、估計尺寸系統性偏大。共用圖禁用令已於 `121697a` 解除（紀錄 `2b88d89`）。→ **Phase 1.10**

## Phase 1.10 — 參數為主、照片為輔（SPEC v0.5；Fable 裁決 T-17-R2-A，2026-09-21）← 現在

目標：把產品改成「照片先填建議值 → 使用者確認／修改 → 出 IR」，並用真實照片做第三次驗收（T-17-R3）。任務卡見 [TASKS.md](TASKS.md)「Phase 1.10」。

- [ ] T-63 🔮 MVP 判準 v2 草案（輔助路徑；門檻數字未放寬）— 等獨立 Opus 審條文＋使用者核准 → `criteria:` commit
- [ ] T-64 真實空間素材（使用者自拍＋實測尺寸＋逐面材質＋選配現場錄音；held-out 從出生就隔離）
      — 🔮 2026-09-21 Fable r1 補（上兩行原字保留）：T-63 已另起「修訂版 r1」待 Opus 審 diff；使用者表示無法到現場拍照／錄音 → T-64 改**路徑 S**（Codex 十張合成圖分組封存；真實素材軌擱置），R3 標題的「用真實照片」改為「held-out＝AI 合成圖（報告強制揭露，不能外推到真實照片）」。
- [ ] T-70 公開小房間 IR 資料集（調查 → 使用者授權下載 → 封存）＝判準 2 名單候選（新，2026-09-21）
- [ ] T-66 gate 出口一律給尺寸導引（百貨賣場缺口）→ [ ] T-65 `--suggest`／`--params` 兩段式 CLI（F-18）
- [ ] T-67 R2 工具防呆（`--help`／未知引數／拒絕覆寫）
- [ ] T-17-R3 第三次驗收（保留號；上列全部完成後由 Fable 落地）
- 改善軌（不擋 R3）：[ ] T-59 ir_synth 鄰帶洩漏診斷（已正式開卡）｜[ ] T-68 材質專用模型可行性量測（需使用者授權下載）｜[ ] T-69 幾何 v2（保留號，等 T-64 開發集）
- 本輪決定不做：陳設公式修正輪（R3 後依事前寫死的觸發條件再議）；`IR_RT60_BASIS` 維持 sabine；gate 不調；T-44-R1／T-53 繼續 ⏸

## Phase 1.5 — 場景描述輸入與複合場景（SPEC F-16 ~ F-17）✅（2026-08-28 全數結案）

目標：不用照片也能產 IR（文字描述），並支援「聲源與聽者在不同空間」的複合場景。
兩者都掛在既有中間表示之上，照片管線不改。任務卡 T-20/T-21/T-22 見 [TASKS.md](TASKS.md)。

- [x] T-20 文字場景描述 → IR（13 種 preset + 關鍵字/參數解析；認不得就報錯列清單）
      ✅ 通過（Opus 驗證 2026-08-28；使用者試聽「沒有問題」）
- [x] T-21 複合場景引擎 v1（路徑串接 + `data/transmission.json` 傳輸損失表；
      示範場景＝巨蛋→走廊、隔壁人聲三路徑）✅ 通過（Opus 複驗 2026-08-28；
      人耳共四輪：v1 退回 → v2 退回 → v3 OK → 修引擎後 v4 OK）
- [x] T-22 T-14 引擎尺度自適應（計畫外新增：T-21 退回時定位到引擎固定 90ms 早期窗
      在 80–120m 跨臨界尺度失效且全程無警示——巨蛋 2k/4k −94%；改為依幾何動態計算，
      驗證至 200m 級、小/中房間 bit-identical 零回歸）✅ 通過（Opus 驗證 2026-08-28）

## Phase 2 — 即時效果器（SPEC F-10 ~ F-15）

目標：可在 DAW 中使用的 AU/VST3 plugin（macOS 優先）。

- [ ] JUCE 專案架構、partitioned convolution 零延遲引擎
- [ ] 參數組：Wet/Dry、Gain、Pre-delay、Size、Damping、EQ
- [ ] Python 管線整合方案定案（ONNX/CoreML 內嵌 vs local service）
- [ ] Plugin 內照片載入 UI 與分析視覺化（v0.5：UI 主體＝參數面板〔空間類型／尺寸／六面材質〕＋preset 瀏覽；照片拖入＝填入建議值，資料模型沿用 F-18 參數檔）
- [ ] IR 庫瀏覽器（內建 preset + 使用者生成）
- [ ] Stereo IR（decorrelation）
- [ ] AU / VST3 打包與 DAW 相容性測試

## Phase 3 — 進階功能（SPEC F-20 ~ F-23）

- [ ] 影片輸入：多幀融合（參考 AV-RIR）。環景輸入已在 Phase 1 解掉「視野外」的一部分，
      影片輸入的價值改為：一般透視照的視野外補全、鏡頭移動時的動態空間
- [ ] Positioner：空間內移動音源（動態早期反射）
- [ ] 神經 IR 生成路線：以聲學參數為條件的生成模型（RESEARCH §2）取代 shaped-noise
- [ ] 空間類型 preset IR 庫（音樂廳、教堂、浴室、車內…）
- [ ] Windows 支援

## 長期願景

- [ ] 即時影片串流 → 動態空間模擬
- [ ] 使用者社群 IR 分享
- [ ] 行動裝置版本（拍照即得該空間 Reverb）
