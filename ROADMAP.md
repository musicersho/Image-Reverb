...
- [ ] T-17 **驗收**：SPEC §7 四項標準（含人耳試聽環節，Opus 主導）
      ...
      照片來源網址為結案前置（裁決 E）
      — ⛔ **首驗結果：MVP FAIL（2026-08-30；§7-1 2/5、§7-2 自動組 0/8／手動組 0/5），永久保留不覆寫；
      重驗＝T-17-R2（裁決 T-45-A，2026-09-03；硬前置 T-42／T-43／T-46／T-47／T-48／T-44-R1 與 held-out 素材）**
      — 🔮 **裁決 T-47-A（2026-09-14，Fable）**：default gate 維持不動；role_aware 0.4 未校準（使用者已核准乙，criteria `81bc4cd` → T-52）；
      T-44-R1 移出關鍵路徑（等 T-53 校準卡）→ T-17-R2 以預設模式重驗，前置改為 T-42／T-43／T-46／裁決 T-47-A／T-48／
      （T-52 結案或選甲）與 held-out 素材
      — 🔮 **裁決 T-48-F（2026-09-14，Fable）**：T-48 A 部分 RacquetballCourt4 域外誤放＝預設路徑安全缺口 → 開 **T-54**（量程規則 v2：環景分支加
      三維檢查）→ **T-55**（T-11 域外 v3 重驗：14 張＋V5 情境）→ 兩者列為 T-17-R2 前置；**T-56**（T-12 v2-b 量測方法 v3：seed 鎖定＋10 次中位數）
      不進關鍵路徑。三份 criteria 草案等使用者核准後才 commit。關鍵路徑：T-48 修正輪 → T-54 → T-55 → T-17-R2（T-52 Opus 複核 4(ii) 平行）
      — ✅ **2026-09-15 使用者核准三份 criteria**（`02284d9`／`b80a4fb`／`4a0b23e`；T-56 守門條款刪）；T-48 工程已驗證（`2ea4d41`）→ **T-54 可開跑**
      — 🔮 **2026-09-16 Fable 落地 T-17-R2**：T-54／T-55／T-56 全部結案；使用者回覆三項（T-04 內部使用不補網址／held-out 建議自拍／P1 依建議落地、貼 Prompt 即核准）；
      in-domain 事前定義（8 場地僅 `mit_gym`）；執行步驟 0～11；§8 前三欄已填。**T-17-R2 等 held-out 照片就位即開跑（Opus 主導）**，期望 `MVP：FAIL（R2）`
      — 🔮 **2026-09-18**：等照片期間開 **T-57**（R2 工具前置，Sonnet→Opus；T-17-R2 前置追加）與 **T-58**（調查卡：Sabine／Eyring／pra 參考／產品路徑四基準對照，只量不改，填充）
      — 🔮 **2026-09-18（同日）**：T-57 首驗 🟠 工程退回 → Fable 裁定報告項 5 分母口徑（裁定 T-57-D，`criteria:` `3007646`，結果前）＋開修正輪 **T-57-F1**（Sonnet→Opus）。關鍵路徑：T-57-F1 → Opus 驗 → T-17-R2（仍等 held-out 照片）；T-58 平行
      — 🔮 **2026-09-18（同日）**：T-58 首驗 🟠 工程退回（量測可重現，判定文字 R1～R5）→ 開修正輪 **T-58-F1**（Sonnet→Opus；只重跑 `report`；不進關鍵路徑）。
      實驗：負向——現有證據不支持把 `IR_RT60_BASIS` 由 sabine 換成 eyring／pra；**產品決定留到 T-17-R2 之後**。`ir_synth` 鄰帶耦合診斷登記保留號 **T-59**（R2 後開卡）
